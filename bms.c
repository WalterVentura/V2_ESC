        ...
        if(is_ext)
        {
            ...
            switch(cmd)
            {
                case CAN_PACKET_BMS_SOC_SOH_TEMP_STAT:
                {
                    ...
                    if((id == m_values.can_id) ||
                       (UTILS_AGE_S(m_values.update_time) > MAX_CAN_AGE_SEC))
                    {
                        m_values.can_id = id;
                ...
                case CAN_PACKET_BMS_BAL:
                {
                       ...
                       (UTILS_AGE_S(m_values.update_time) > MAX_CAN_AGE_SEC))
                    {
                        ...
                        while(ind < (int) (sizeof(m_values.bal_state) / sizeof(bool)) &&
                              ind < cell_num)
                        {
...
void bms_update_limits(float* i_in_min, float* i_in_max, float i_in_min_conf, float i_in_max_conf)
{
...
void bms_process_cmd(unsigned char* data, unsigned int len, void (* reply_func)(unsigned char* data,
                                                                                unsigned int len))
{
...
void bms_send_status_can(void)
{
    ...
    buffer[send_index++] = 0;
    comm_can_transmit_eid(id | ((uint32_t) CAN_PACKET_BMS_SOC_SOH_TEMP_STAT << 8), buffer,
                          send_index);
    ...
    comm_can_transmit_eid(id | ((uint32_t) CAN_PACKET_BMS_AH_WH_CHG_TOTAL << 8), buffer,
                          send_index);
    ...
    comm_can_transmit_eid(id | ((uint32_t) CAN_PACKET_BMS_AH_WH_DIS_TOTAL << 8), buffer,
                          send_index);
}
