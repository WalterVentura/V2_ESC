...
void hw_init_gpio(void)
{
    ...
    // LEDs
    palSetPadMode(GPIOE, 10,
                  PAL_MODE_OUTPUT_PUSHPULL |
                  PAL_STM32_OSPEED_HIGHEST);
    palSetPadMode(GPIOE, 12,
                  PAL_MODE_OUTPUT_PUSHPULL |
                  PAL_STM32_OSPEED_HIGHEST);

    // GPIOC (ENABLE_GATE)
    palSetPadMode(GPIOG, 6,
                  PAL_MODE_OUTPUT_PUSHPULL |
                  PAL_STM32_OSPEED_HIGHEST);
    ...
    // Fault pin
    palSetPadMode(GPIOF, 14, PAL_MODE_INPUT_PULLUP);
...
void hw_setup_adc_channels(void)
{
    ...
    ADC_RegularChannelConfig(ADC3, ADC_Channel_0, 3, ADC_SampleTime_15Cycles);
    ADC_RegularChannelConfig(ADC3, ADC_Channel_14, 4, ADC_SampleTime_15Cycles);
