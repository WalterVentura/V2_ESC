...
void hw_init_gpio(void)
{
    ...
    palSetPadMode(SPI_SW_SCK_GPIO, SPI_SW_SCK_PIN,
                  PAL_MODE_OUTPUT_PUSHPULL | PAL_STM32_OSPEED_HIGHEST);
    palSetPadMode(SPI_SW_FPGA_CS_GPIO, SPI_SW_FPGA_CS_PIN,
                  PAL_MODE_OUTPUT_PUSHPULL | PAL_STM32_OSPEED_HIGHEST);
    palSetPadMode(SPI_SW_MOSI_GPIO, SPI_SW_MOSI_PIN,
                  PAL_MODE_OUTPUT_PUSHPULL | PAL_STM32_OSPEED_HIGHEST);
    ...
    // FPGA RESET
    palSetPadMode(AXIOM_FPGA_RESET_PORT, AXIOM_FPGA_RESET_PIN,
                  PAL_MODE_OUTPUT_PUSHPULL | PAL_STM32_OSPEED_HIGHEST);
...
char hw_axiom_configure_FPGA(void)
{
    ...
    for(int i = 0; i < chunks; i++)
    {
        ...
        r = lzo1x_decompress_safe(FPGA_bitstream + index, compressed_chunk_size, outputBuffer,
...
static void terminal_cmd_store_current_sensor_gain(int argc, const char** argv)
{
    ...
    if(argc == 2)
    {
        ...
        {
            commands_printf("Axiom current sensor sensor gain set as %.8f",
...
static void terminal_cmd_store_input_current_sensor_gain(int argc, const char** argv)
{
    ...
    if(argc == 2)
    {
        ...
        if((current_gain.as_float > 0.0) && (current_gain.as_float < 1.0))
        {
            ...
            {
                commands_printf("Axiom input current sensor sensor gain set as %.8f",
                                (double) input_current_sensor_gain);
            }
            else
            {
...
static void terminal_cmd_read_current_sensor_gain(int argc, const char** argv)
{
    ...
    commands_printf("Axiom input current sensor gain is set as %.8f",
                    (double) input_current_sensor_gain);
...
float hw_axiom_read_input_current(void)
{
    ...
    {
        ret_value =
            ((V_REG / 4095.0) * (float) ADC_Value[ADC_IND_EXT2] - input_current_sensor_offset) /
            input_current_sensor_gain;
    }
...
float hw_axiom_temp_sensor_filter(uint8_t temp_sensor)
{
    ...
    switch(temp_sensor)
    {
        case 1:
            UTILS_LP_FAST(temp1,
                          (1.0 /
                           ((logf(NTC_RES_IGBT(ADC_Value[ADC_IND_TEMP_IGBT_1]) / 5000.0) / 3433.0) +
                            (1.0 / 298.15)) -
                           273.15),
                          0.01);
        ...
        case 2:
            UTILS_LP_FAST(temp2,
                          (1.0 /
                           ((logf(NTC_RES_IGBT(ADC_Value[ADC_IND_TEMP_IGBT_2]) / 5000.0) / 3433.0) +
                            (1.0 / 298.15)) -
                           273.15),
                          0.01);
        ...
        case 3:
            UTILS_LP_FAST(temp3,
                          (1.0 /
                           ((logf(NTC_RES_IGBT(ADC_Value[ADC_IND_TEMP_IGBT_3]) / 5000.0) / 3433.0) +
                            (1.0 / 298.15)) -
                           273.15),
                          0.01);
...
void terminal_cmd_axiom_print_log(int argc, const char** argv)
{
    ...
    commands_printf("Axiom FW %d.%d BETA %d", FW_VERSION_MAJOR, FW_VERSION_MINOR,
    ...
    commands_printf(
        "- 2023-06-22 Add empty SPI message and angle velocity error, add peak values for error rates");
