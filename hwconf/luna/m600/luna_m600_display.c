...
static volatile luna_settings_t luna_settings =
{
    .light_mode = LUNA_LIGHT_MODE_OFF,
...
{
    return luna_settings.pas_torque /
           (LUNA_TORQUE_SENSOR_MAXIMUM_TORQUE - LUNA_TORQUE_SENSOR_MINIMUM_TORQUE);
}
...
static void set_assist_level(uint8_t assist_code)
{
    ...
    switch(assist_code)
    {
        ...
        case PAS_LEVEL_1:
            current_scale = 1.0 / 9.0;
            break;

        case PAS_LEVEL_2:
            current_scale = 2.0 / 9.0;
            break;

        case PAS_LEVEL_3:
            current_scale = 3.0 / 9.0;
            break;

        case PAS_LEVEL_4:
            current_scale = 4.0 / 9.0;
            break;

        case PAS_LEVEL_5:
            current_scale = 5.0 / 9.0;
            break;

        case PAS_LEVEL_6:
            current_scale = 6.0 / 9.0;
            break;

        case PAS_LEVEL_7:
            current_scale = 7.0 / 9.0;
            break;

        case PAS_LEVEL_8:
            current_scale = 8.0 / 9.0;
            break;
    ...
    if(delay_between_states > 50)
    {
        ...
        switch(can_display_process_state)
        {
            case SEND_BATTERY_RANGE_STATE:
            {
                ...
                {
                    time_at_last_distance_change = (float) chVTGetSystemTimeX() /
        ...
        case LUNA_TORQUE_SENSOR_ID:
        {
            if(len == LUNA_TORQUE_SENSOR_ID_LENGTH_BYTES)
            {
                if(data != NULL)
                {
                    ...
                    uint16_t torque_raw_level =
                        (((uint16_t) data[1] << 8) & 0xff00) | ((uint16_t) data[0] & 0x00ff);
...
static THD_FUNCTION(display_process_thread, arg)
{
    ...
    for(;;)
    {
        ...
        if(encoder_recovery_done)
        {
            ...
            if(mc_interface_get_configuration()->foc_encoder_offset ==
               400.0 /*|| fabsf(hw_get_encoder_error()) > 10.0*/)
            {
