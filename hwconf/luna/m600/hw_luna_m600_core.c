...
void hw_init_gpio(void)
{
    ...
    palSetPadMode(GPIOA, 8, PAL_MODE_ALTERNATE(
                      GPIO_AF_TIM1) | PAL_STM32_OSPEED_HIGHEST | PAL_STM32_PUDR_FLOATING);
    palSetPadMode(GPIOA, 9, PAL_MODE_ALTERNATE(
                      GPIO_AF_TIM1) | PAL_STM32_OSPEED_HIGHEST | PAL_STM32_PUDR_FLOATING);
    palSetPadMode(GPIOA, 10, PAL_MODE_ALTERNATE(
                      GPIO_AF_TIM1) | PAL_STM32_OSPEED_HIGHEST | PAL_STM32_PUDR_FLOATING);

    palSetPadMode(GPIOB, 13, PAL_MODE_ALTERNATE(
                      GPIO_AF_TIM1) | PAL_STM32_OSPEED_HIGHEST | PAL_STM32_PUDR_FLOATING);
    palSetPadMode(GPIOB, 14, PAL_MODE_ALTERNATE(
                      GPIO_AF_TIM1) | PAL_STM32_OSPEED_HIGHEST | PAL_STM32_PUDR_FLOATING);
    palSetPadMode(GPIOB, 15, PAL_MODE_ALTERNATE(
                      GPIO_AF_TIM1) | PAL_STM32_OSPEED_HIGHEST | PAL_STM32_PUDR_FLOATING);
    ...
    palSetPadMode(HW_PHASE_A_FILTER_GPIO, HW_PHASE_A_FILTER_PIN,
                  PAL_MODE_OUTPUT_OPENDRAIN | PAL_STM32_OSPEED_HIGHEST);
    palSetPadMode(HW_PHASE_B_FILTER_GPIO, HW_PHASE_B_FILTER_PIN,
                  PAL_MODE_OUTPUT_OPENDRAIN | PAL_STM32_OSPEED_HIGHEST);
    palSetPadMode(HW_PHASE_C_FILTER_GPIO, HW_PHASE_C_FILTER_PIN,
                  PAL_MODE_OUTPUT_OPENDRAIN | PAL_STM32_OSPEED_HIGHEST);
    PHASE_FILTER_OFF();
#else

    // MT6816 program enable
    palSetPadMode(MT6816_PROG_EN_GPIO, MT6816_PROG_EN_PIN,
                  PAL_MODE_OUTPUT_PUSHPULL | PAL_STM32_OSPEED_HIGHEST);
    MT6816_PROG_DISABLE();
#endif
...
float hw_get_mosfet_temp_filtered(void)
{
    ...
    float mosfet_temp =
        (1.0 / ((logf(NTC_RES(ADC_Value[ADC_IND_TEMP_MOS]) / 10000.0) / 3455.0) + (1.0 / 298.15)) -
         273.15);
...
{
    eeprom_var use_fixed_throttle;
    bool var_not_found = !conf_general_read_eeprom_var_hw(&use_fixed_throttle,
                                                          EEPROM_ADDR_FIXED_THROTTLE_LEVEL);
...
float hw_get_encoder_error(void)
{
    ...
    {
        angle_diff = utils_angle_difference(
            mcpwm_foc_get_phase_encoder(), mcpwm_foc_get_phase_observer());
    }
...
void hw_recover_encoder_offset(void)
{
    ...
    else
    {
        ...
        if(mc_conf_encoder_offset <= 360.0)
        {
            ...
            if((m600_backup->encoder_offset_calibration_done == false) ||
               (mc_conf_encoder_offset != m600_backup->encoder_offset))
            {
