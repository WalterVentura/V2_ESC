...
int8_t hw_read_initial_assist_level(void)
{
    ...
    bool var_not_found = !conf_general_read_eeprom_var_hw(&assist_level,
                                                          EEPROM_ADDR_INITIAL_ASSIST_LEVEL);
...
float hw_read_motor_temp(float beta)
{
    ...
    else
    {
        // NTC
        return (1.0 /
                ((logf(NTC_RES_MOTOR(ADC_Value[ADC_IND_TEMP_MOTOR]) / 10000.0) / beta) +
                 (1.0 / 298.15)) -
                273.15);
    }
}
...
bool hw_bbshd_has_PTC_sensor(void)
{
    ...
    // return 1;
    bool var_not_found =
        !conf_general_read_eeprom_var_hw(&has_ptc, EEPROM_ADDR_MOTOR_HAS_PTC_SENSOR);
