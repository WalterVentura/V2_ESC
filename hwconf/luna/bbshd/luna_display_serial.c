        ...
        if(serial_buffer.data[rd_ptr] == CMD_WRITE)
        {
            ...
            switch(command)
            {
                ...
                case CMD_WRITE_BASIC:
                    ...
                    if(checksum_addr <= serial_buffer.wr_ptr) // check the checksum has been received
                    {
                        commands_printf("checksum received,calculated: %d    %d",
                                        serial_buffer.data[checksum_addr],
                                        checksum(serial_buffer.data + rd_ptr,
                                                 CMD_WRITE_BASIC_LENGTH - 1));
                           ...
                           checksum(serial_buffer.data + rd_ptr, CMD_WRITE_BASIC_LENGTH - 1))
                        {
                            ...
                            mcconf->si_wheel_diameter = (float) serial_buffer.data[rd_ptr + 24] *
                                                        25.4 / 2.0;
 ...
 *                  continue; */
        ...
        if(serial_buffer.data[rd_ptr] == CMD_READ)
        {
            ...
            switch(command)
            {
                ...
                case CMD_READ_BASIC:
                {
                    ...
                    serial_buffer.tx[26] =
                        checksum(serial_buffer.tx + 2, CMD_READ_BASIC_LENGTH - 3);
                ...
                case CMD_WHEEL_RPM:
                {
                    ...
                    uint16_t wheel_rpm =
                        (uint16_t) (mc_interface_get_speed() / (M_PI * conf->si_wheel_diameter) *
                                    60.0);
    ...
    {
        memmove(serial_buffer.data, serial_buffer.data + serial_buffer.rd_ptr,
                LUNA_SERIAL_BUFFER_SIZE - serial_buffer.rd_ptr);
