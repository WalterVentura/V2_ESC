...
void hw_setup_adc_channels(void)
{
    ...
    chThdCreateStatic(switch_color_thread_wa, sizeof(switch_color_thread_wa), LOWPRIO,
                      switch_color_thread, NULL);
}
...
{
    if((palReadPad(SWITCH_IN_GPIO,
                   SWITCH_IN_PIN) == 1) && (mc_interface_temp_fet_filtered() < 68.0))
    {
        return true;
    }
    else
    ...
    for(;;)
    {
        switch(switch_state)
        {
            ...
            case SWITCH_TURN_ON_DELAY_ACTIVE:
                ...
                while((ADC_Value[ADC_IND_V_BATT] < 1 || ADC_Value[ADC_IND_VIN_SENS] < 1) &&
                      (cts < 50))
                {
...
void smart_switch_pin_init(void)
{
    ...
    palSetPadMode(SWITCH_OUT_GPIO, SWITCH_OUT_PIN,
                  PAL_MODE_OUTPUT_PUSHPULL | PAL_STM32_OSPEED_HIGHEST);
    palSetPadMode(SWITCH_LED_1_GPIO, SWITCH_LED_1_PIN,
                  PAL_MODE_OUTPUT_OPENDRAIN | PAL_STM32_OSPEED_HIGHEST);
    palSetPadMode(SWITCH_LED_2_GPIO, SWITCH_LED_2_PIN,
                  PAL_MODE_OUTPUT_OPENDRAIN | PAL_STM32_OSPEED_HIGHEST);
    palSetPadMode(SWITCH_LED_3_GPIO, SWITCH_LED_3_PIN,
                  PAL_MODE_OUTPUT_OPENDRAIN | PAL_STM32_OSPEED_HIGHEST);
    palSetPadMode(SWITCH_PRECHARGED_GPIO, SWITCH_PRECHARGED_PIN,
                  PAL_MODE_OUTPUT_PUSHPULL | PAL_STM32_OSPEED_HIGHEST);
    palClearPad(SWITCH_PRECHARGED_GPIO, SWITCH_PRECHARGED_PIN);
    palSetPad(SWITCH_OUT_GPIO, SWITCH_OUT_PIN);
    LED_SWITCH_B_ON();
