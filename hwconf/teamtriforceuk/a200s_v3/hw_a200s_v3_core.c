...
float hw_a200s_get_temp_mos(void)
{
    ...
    float mos_temp = t_board +
                     ((SQ(motor_current_now) * HW_TEMP_MOSFET_R / HW_TEMP_MOSFET_COUNT) /
                      HW_TEMP_NUM_PHASES *
                      HW_TEMP_MOSFET_KW);
    ...
    mos_temp = t_board +
               ((SQ(motor_current_now) * mos_r_temp_comp / HW_TEMP_MOSFET_COUNT) /
                HW_TEMP_NUM_PHASES *
                HW_TEMP_MOSFET_KW);
...
float hw_a200s_get_temp_shunt(void)
{
    ...
    float shunt_temp = t_board +
                       ((((SQ(input_current_now) * CURRENT_SHUNT_RES) / HW_TEMP_NUM_PHASES)) *
                        HW_TEMP_MOSFET_KW);
...
static void terminal_cmd_doublepulse(int argc, const char** argv)
{
    ...
    {
        TIM_BDTRInitStructure.TIM_DeadTime = conf_general_calculate_deadtime(deadtime,
                                                                             SYSTEM_CORE_CLOCK);
    }
