...
static lbm_value ext_rb_read_brake(lbm_value* args, lbm_uint argn)
{
    (void) args;
    (void) argn;

    return lbm_enc_i(BRAKE_SW_ACTIVE() ? 1 : 0);
}

static lbm_value ext_rb_set_front_light(lbm_value* args, lbm_uint argn)
{
...
static lbm_value ext_rb_set_rear_light(lbm_value* args, lbm_uint argn)
{
    LBM_CHECK_ARGN_NUMBER(1);
...
void hw_init_gpio(void)
{
    ...
    palSetPadMode(FRONT_LIGHT_GPIO, FRONT_LIGHT_PIN,
                  PAL_MODE_OUTPUT_PUSHPULL | PAL_STM32_OSPEED_HIGHEST);
    palSetPadMode(REAR_LIGHT_GPIO, REAR_LIGHT_PIN,
                  PAL_MODE_OUTPUT_PUSHPULL | PAL_STM32_OSPEED_HIGHEST);
    FRONT_LIGHT_OFF();
    REAR_LIGHT_OFF();
