...
#define HW_DEFAULT_ID             (APPCONF_CONTROLLER_ID >= \
                                   0 ? APPCONF_CONTROLLER_ID : hw_id_from_uuid())
#endif
...
#define PTC_TEMP_MOTOR(res, con,                                                                     \
                       tbase)             (((NTC_RES_MOTOR(ADC_Value[ADC_IND_TEMP_MOTOR]) - (res)) / \
                                            (res)) * 100 / (con) +                                   \
                                           (tbase))
#define PTC_TEMP_MOTOR_2(res, con,                                                            \
                         tbase)           (((NTC_RES_MOTOR(ADC_Value[ADC_IND_TEMP_MOTOR_2]) - \
                                             (res)) / (res)) * 100 / (con) +                  \
                                           (tbase))
#else
...
#if defined(NTC_RES_MOTOR) && defined(ADC_IND_TEMP_MOTOR)
#define NTC100K_TEMP_MOTOR(beta)   (1.0 /                                                 \
                                    ((logf(NTC_RES_MOTOR(ADC_Value[ADC_IND_TEMP_MOTOR]) / \
                                           100000.0) / beta) + (1.0 / 298.15)) -          \
                                    273.15)
#define NTC100K_TEMP_MOTOR_2(beta) (1.0 /                                                   \
                                    ((logf(NTC_RES_MOTOR(ADC_Value[ADC_IND_TEMP_MOTOR_2]) / \
                                           100000.0) / beta) + (1.0 / 298.15)) -            \
                                    273.15)
#else
...
#if defined(NTC_RES_MOTOR) && defined(ADC_IND_TEMP_MOTOR)
#define NTCX_TEMP_MOTOR(res, beta,                                                           \
                        tbase)              (1.0 /                                           \
                                             ((logf(NTC_RES_MOTOR(ADC_Value[                 \
                                                                      ADC_IND_TEMP_MOTOR]) / \
                                                    (res)) / (beta)) +                       \
                                              (1.0 / (273.15 + (tbase)))) -                  \
                                             273.15)
#define NTCX_TEMP_MOTOR_2(res, beta,                                                           \
                          tbase)            (1.0 /                                             \
                                             ((logf(NTC_RES_MOTOR(ADC_Value[                   \
                                                                      ADC_IND_TEMP_MOTOR_2]) / \
                                                    (res)) / (beta)) +                         \
                                              (1.0 / (273.15 + (tbase)))) -                    \
                                             273.15)
#else
