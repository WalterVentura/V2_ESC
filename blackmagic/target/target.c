...
static int target_flash_write_buffered(struct target_flash* f, target_addr dest, const void* src,
                                       size_t len);
...
int target_flash_write_buffered(struct target_flash* f, target_addr dest, const void* src,
                                size_t len)
{
    int ret = 0;
...
int target_breakwatch_set(target* t, enum target_breakwatch type, target_addr addr, size_t len)
{
...
int target_breakwatch_clear(target* t, enum target_breakwatch type, target_addr addr, size_t len)
{
...
int tc_open(target* t, target_addr path, size_t plen, enum target_open_flags flags, mode_t mode)
{
    if(t->tc->open == NULL)
    {
