...
static const char tdesc_cortex_m[] = "No desc";
...
static const char tdesc_cortex_mf[] = "No desc";
...
enum
{
    DB_DHCSR, DB_DCRSR, DB_DCRDR, DB_DEMCR
};
...
int cortexm_mem_write_sized(target* t, target_addr dest, const void* src, size_t len,
                            enum align align)
{
    cortexm_cache_clean(t, dest, len, true);
...
int cortexm_run_stub(target* t, uint32_t loadaddr, uint32_t r0, uint32_t r1, uint32_t r2,
                     uint32_t r3)
{
    uint32_t regs[t->regs_size / 4];
