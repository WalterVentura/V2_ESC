...
static void terminal_map_nrf5_pins(int argc, const char** argv)
{
    ...
    if(argc == 2)
    {
        ...
        {
            bm_change_swd_pins(NRF5x_SWDIO_GPIO, NRF5x_SWDIO_PIN, NRF5x_SWCLK_GPIO,
                               NRF5x_SWCLK_PIN);
 ...
 * new SWCLK pin
 */
void bm_change_swd_pins(stm32_gpio_t* swdio_port, int swdio_pin, stm32_gpio_t* swclk_port,
                        int swclk_pin)
{
    bm_set_enabled(false);
