...
void terminal_process_string(char* str)
{
    ...
    {
        commands_printf("Latest ADC duration: %.4f ms",
                        (double) (mcpwm_get_last_adc_isr_duration() * 1000.0));
        commands_printf("Latest injected ADC duration: %.4f ms",
                        (double) (mc_interface_get_last_inj_adc_isr_duration() * 1000.0));
    ...
    else if(strcmp(argv[0], "threads") == 0)
    {
        ...
        do {
            ...
            commands_printf("%.8lx %.8lx %4lu %4lu %9s %14s %5lu %8d  %lu (%.1f %%)",
                            ...
                            states[tp->p_state], tp->p_name, tp->motor_selected, stack_left,
                            (uint32_t) tp->p_time,
                            (double) (100.0 * (float) tp->p_time /
        ...
        else
        {
            ...
            for(int i = 0; i < fault_vec_write; i++)
            {
                commands_printf("Fault            : %s",
                                mc_interface_fault_to_string(fault_vec[i].fault));
                ...
                commands_printf("Gate drv voltage : %.2f",
                                (double) fault_vec[i].gate_driver_voltage);
#endif
                ...
                commands_printf("TIM duty         : %d",
                                (int) ((float) fault_vec[i].tim_top * fault_vec[i].duty));
                ...
                {
                    commands_printf("DRV8301_FAULTS   : %s",
                                    drv8301_faults_to_string(fault_vec[i].drv8301_faults));
                }
                ...
                if(fault_vec[i].fault == FAULT_CODE_DRV)
                {
                    commands_printf("DRV8320S_FAULTS  : %s",
                                    drv8320s_faults_to_string(fault_vec[i].drv8301_faults));
                }
                ...
                if(fault_vec[i].fault == FAULT_CODE_DRV)
                {
                    commands_printf("DRV8323S_FAULTS  : %s",
                                    drv8323s_faults_to_string(fault_vec[i].drv8301_faults));
                }
                ...
                if(fault_vec[i].info_str != 0)
                {
                    ...
                    {
                        commands_printf(f_str, (double) fault_vec[i].info_args[0],
                                        (double) fault_vec[i].info_args[1]);
                    }
                }
    ...
    else if(strcmp(argv[0], "param_detect") == 0)
    {
        ...
        if(argc == 4)
        {
               ...
               (low_duty > 0.02) && (low_duty < 0.8))
            {
                ...
                if(conf_general_detect_motor_param(current, min_rpm, low_duty, &cycle_integrator,
                                                   &coupling_k, hall_table, &hall_res))
                {
            ...
            else
            {
                ...
                {
                    commands_printf("Current must be between 0.0 and %.2f",
        ...
        if(argc == 2)
        {
            ...
            *mcconf_old = *mc_interface_get_configuration();
            ...
            if((current > 0.0) && (current <= mcconf->l_current_max))
            {
                ...
                {
                    commands_printf("Resistance measurement failed due to fault: %s", mc_interface_fault_to_string(
                                        fault));
        ...
        if(argc == 2)
        {
            ...
            if((duty > 0.0) && (duty <= 0.9))
            {
                ...
                *mcconf_old = *mc_interface_get_configuration();
                ...
                {
                    commands_printf("Inductance measurement failed with fault: %s", mc_interface_fault_to_string(
                                        fault));
                    commands_printf("For more info type \"faults\" to view all logged faults\n");
                }
    ...
    else if(strcmp(argv[0], "measure_linkage") == 0)
    {
        ...
        {
            commands_printf(
                "This command requires four arguments. [current duty min_erpm resistance]\n");
        }
    }
    else if(strcmp(argv[0], "measure_res_ind") == 0)
    {
        ...
        *mcconf_old = *mc_interface_get_configuration();
        ...
        {
            commands_printf("Fault occured while measuring resistance and inductance: %s", mc_interface_fault_to_string(
                                fault));
            commands_printf("For more info type \"faults\" to view all logged faults\n");
        }
        ...
        if(argc == 2)
        {
            ...
            if((duty > 0.0) && (duty <= 0.9))
            {
                ...
                *mcconf_old = *mc_interface_get_configuration();
                ...
                {
                    commands_printf("Flux linkage detection failed with fault: %s", mc_interface_fault_to_string(
                                        fault));
                    commands_printf("For more info type \"faults\" to view all logged faults\n");
                }
            }
    ...
    else if(strcmp(argv[0], "measure_linkage_openloop") == 0)
    {
        if(argc == 6)
        {
            ...
            if((current > 0.0) && (current <= mc_interface_get_configuration()->l_current_max) &&
               (erpm_per_sec > 0.0) && (duty > 0.02) && (duty <= 0.9) && (res >= 0.0) &&
               (ind >= 0.0))
            {
                float linkage = 0.0, linkage_undriven = 0.0, undriven_samples = 0.0;
                bool result;

                int fault = conf_general_measure_flux_linkage_openloop(current, duty, erpm_per_sec,
                                                                       res, ind,
                                                                       &linkage, &linkage_undriven,
                                                                       &undriven_samples, &result);
                ...
                {
                    commands_printf("Fault occured while measuring flux linkage: %s", mc_interface_fault_to_string(
                                        fault));
                    commands_printf("For more info type \"faults\" to view all logged faults\n");
                }
            }
        ...
        {
            commands_printf(
                "This command requires five arguments. [current duty erpm_ramp_per_sec resistance inductance]\n");
        }
    }
    ...
    else if(strcmp(argv[0], "hw_status") == 0)
    {
        ...
        commands_printf("Current Measurement Range: %.1f A",
                        (double) ((V_REG / 2.0) / (CURRENT_AMP_GAIN * CURRENT_SHUNT_RES)));
#endif

#if defined(V_REG) && defined(VIN_R1) && defined(VIN_R2)
        commands_printf("Voltage Measurement Range: %.1f V",
                        (double) ((V_REG / 4095.0) * 4095.0 * ((VIN_R1 + VIN_R2) / VIN_R2)));
#endif
        ...
        commands_printf("Mempool mcconf now: %d highest: %d (max %d)",
                        mempools_mcconf_allocated_num(),
                        mempools_mcconf_highest(), MEMPOOLS_MCCONF_NUM - 1);
        commands_printf("Mempool appconf now: %d highest: %d (max %d)",
                        mempools_appconf_allocated_num(),
                        mempools_appconf_highest(), MEMPOOLS_APPCONF_NUM - 1);
    ...
    else if(strcmp(argv[0], "foc_openloop") == 0)
    {
        if(argc == 3)
        {
               ...
               (erpm >= 0.0))
            {
                ...
                if(fault != FAULT_CODE_NONE)
                {
                    commands_printf("Fault occured during openloop: %s", mc_interface_fault_to_string(
                                        fault));
                    commands_printf("For more info type \"faults\" to view all logged faults\n");
                }
            }
    ...
    else if(strcmp(argv[0], "foc_sensors_detect_apply") == 0)
    {
        if(argc == 2)
        {
            ...
            if((current > 0.0) && (current <= mc_interface_get_configuration()->l_current_max))
            {
                ...
                {
                    commands_printf("Fault occured while detecting sensors: %s", mc_interface_fault_to_string(
                                        fault));
                    commands_printf("For more info type \"faults\" to view all logged faults\n");
                }
            }
    ...
    else if(strcmp(argv[0], "rotor_lock_openloop") == 0)
    {
        if(argc == 4)
        {
                    ...
                    for(float t = 0.0; t < time; t += 0.002)
                    {
                        ...
                        if(fault != FAULT_CODE_NONE)
                        {
                            commands_printf("Fault occured during openloop: %s", mc_interface_fault_to_string(
                                                fault));
                            commands_printf(
                                "For more info type \"faults\" to view all logged faults\n");
                            return;
                        }
            ...
            else
            {
                ...
                if(!(fabsf(current) <= mc_interface_get_configuration()->l_current_max))
                {
                    commands_printf("Current must be less than %.2f",
                                    (double) mc_interface_get_configuration()->l_current_max);
                }
            ...
            if(max_power_loss > 0.0)
            {
                ...
                if(res >= 0)
                {
                    ...
                    commands_printf("Motor Current       : %.1f A",
                                    (double) (mcconf->l_current_max));
                    commands_printf("Motor R             : %.2f mOhm",
                                    (double) (mcconf->foc_motor_r * 1e3));
                    commands_printf("Motor L             : %.2f uH",
                                    (double) (mcconf->foc_motor_l * 1e6));
                    commands_printf("Motor Flux Linkage  : %.3f mWb",
                                    (double) (mcconf->foc_motor_flux_linkage * 1e3));
                    commands_printf("Temp Comp           : %s",
                                    mcconf->foc_temp_comp ? "true" : "false");

                    if(mcconf->foc_temp_comp)
                    {
                        commands_printf("Temp Comp Base Temp : %.1f degC",
                                        (double) mcconf->foc_temp_comp_base_temp);
                    }
    ...
    else if(strcmp(argv[0], "foc_detect_apply_all_can") == 0)
    {
        if(argc == 2)
        {
            ...
            if(max_power_loss > 0.0)
            {
                int res = conf_general_detect_apply_all_foc_can(true, max_power_loss, 0.0, 0.0, 0.0,
                                                                0.0, NULL);
                ...
                if(res >= 0)
                {
                    ...
                    commands_printf("Motor R             : %.2f mOhm",
                                    (double) (mcconf->foc_motor_r * 1e3));
                    commands_printf("Motor L             : %.2f microH",
                                    (double) (mcconf->foc_motor_l * 1e6));
                    commands_printf("Motor Flux Linkage  : %.3f mWb",
                                    (double) (mcconf->foc_motor_flux_linkage * 1e3));
                    commands_printf("Temp Comp           : %s",
        ...
        if(argc == 2)
        {
            ...
            if((current > 0.0) && (current <= mc_interface_get_configuration()->l_current_max))
            {
                ...
                *mcconf = *mc_interface_get_configuration();
                ...
                if(fault != FAULT_CODE_NONE)
                {
                    commands_printf("Fault occured while analyzing hall sensors: %s", mc_interface_fault_to_string(
                                        fault));
                    commands_printf("For more info type \"faults\" to view all logged faults\n");
                }
                else
                {
                    ...
                    {
                        commands_printf(
                            "Found %d different states. Something is most likely wrong...\n",
                            state_num);
                    }
                    ...
                    for(int i = 0; i < 6; i++)
                    {
                        float diff = fabsf(utils_angle_difference(transitions[i],
                                                                  transitions[i + 1]));
                    ...
                    {
                        commands_printf(
                            "Maximum deviation: %.2f degrees. This is good alignment.\n",
                            (double) deviation);
                    }
                    else if((max - min) < 10)
                    {
                        commands_printf(
                            "Maximum deviation: %.2f degrees. This is OK, but not great alignment.\n",
                            (double) deviation);
                    }
                    else if((max - min) < 15)
                    {
                        commands_printf(
                            "Maximum deviation: %.2f degrees. This is bad, but probably usable alignment.\n",
                            (double) deviation);
                    }
                    else
                    {
...
void terminal_register_command_callback(const char* command, const char* help,
                                        const char* arg_names, void (* cbf)(int argc,
                                                                            const char** argv))
{
