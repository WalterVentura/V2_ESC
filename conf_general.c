...
void conf_general_init(void)
{
    ...
    if(!is_ok)
    {
        ...
        {
            memcpy((void*) backup_tmp.hw_config, (uint8_t*) g_backup.hw_config,
                   sizeof(g_backup.hw_config));
        }
    }
...
bool conf_general_detect_motor_param(float current, float min_rpm, float low_duty, float* int_limit,
                                     float* bemf_coupling_k, int8_t* hall_table, int* hall_res)
{
    int ok_steps = 0;
 ...
 */
bool conf_general_measure_flux_linkage(float current, float duty, float min_erpm, float res,
                                       float* linkage)
{
    mc_configuration* mcconf = mempools_alloc_mcconf();
    mc_configuration* mcconf_old = mempools_alloc_mcconf();
 ...
 * Fault code
 */
int conf_general_measure_flux_linkage_openloop(float current, float duty, float erpm_per_sec,
                                               float res, float ind, float* linkage,
    ...
    *mcconf_old = *mcconf;
    ...
    if(fabsf(current) > mcconf->cc_min_current)
    {
        ...
        while(fabsf(mc_interface_get_duty_cycle_now()) < duty)
        {
            ...
            mc_interface_lock_override_once();
            mc_interface_set_openloop_current(current,
                                              mcconf->m_invert_direction ? -rpm_now : rpm_now);
 ...
 * The fault code
 */
int conf_general_autodetect_apply_sensors_foc(float current, bool store_mcconf_on_success,
                                              bool send_mcconf_on_success, int* result)
{
    *result = -1;
    int fault = FAULT_CODE_NONE;
    mc_configuration* mcconf = mempools_alloc_mcconf();
...
static int measure_r_l_imax(float current_min, float current_max, float max_power_loss, float* r,
                            float* l, float* ld_lq_diff, float* i_max)
{
 ...
 */
int conf_general_detect_apply_all_foc(float max_power_loss, bool store_mcconf_on_success,
                                      bool send_mcconf_on_success)
{
    int result = -1;
    ...
    *mcconf_old_second = *mcconf_second;
    ...
    if(res && res_linkage_m2)
    {
        ...
        faultM1 = conf_general_autodetect_apply_sensors_foc(i_max / 3.0,
                                                            store_mcconf_on_success,
                                                            send_mcconf_on_success, &result);
...
int conf_general_detect_apply_all_foc_can(bool detect_can, float max_power_loss,
                                          ...
                                          float openloop_rpm, float sl_erpm, void (* reply_func)(
    ...
    if(detect_can)
    {
        for(int i = 0; i < 255; i++)
        {
            ...
            if(comm_can_ping(i, &hw_type))
            {
                ...
                comm_can_conf_current_limits_in(i, false, mcconf->l_in_current_min,
