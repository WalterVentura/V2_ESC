...
static void process_packet_1(unsigned char* data, unsigned int len)
{
    process_packet(data, len, UART_PORT_COMM_HEADER);
}

static void write_packet_1(unsigned char* data, unsigned int len)
{
    write_packet(data, len, UART_PORT_COMM_HEADER);
}

static void send_packet_1(unsigned char* data, unsigned int len)
{
    app_uartcomm_send_packet(data, len, UART_PORT_COMM_HEADER);
}

static void process_packet_2(unsigned char* data, unsigned int len)
{
    process_packet(data, len, UART_PORT_BUILTIN);
}

static void write_packet_2(unsigned char* data, unsigned int len)
{
    write_packet(data, len, UART_PORT_BUILTIN);
}

static void send_packet_2(unsigned char* data, unsigned int len)
{
    app_uartcomm_send_packet(data, len, UART_PORT_BUILTIN);
}

static void process_packet_3(unsigned char* data, unsigned int len)
{
    process_packet(data, len, UART_PORT_EXTRA_HEADER);
}

static void write_packet_3(unsigned char* data, unsigned int len)
{
    write_packet(data, len, UART_PORT_EXTRA_HEADER);
}

static void send_packet_3(unsigned char* data, unsigned int len)
{
    app_uartcomm_send_packet(data, len, UART_PORT_EXTRA_HEADER);
}
...
void app_uartcomm_initialize(void)
{
    ...
    RxGpioPort[0] = HW_UART_RX_PORT;
    RxGpioPin[0] = HW_UART_RX_PIN;
    TxGpioPort[0] = HW_UART_TX_PORT;
    TxGpioPin[0] = HW_UART_TX_PIN;
    ...
    RxGpioPort[1] = HW_UART_P_RX_PORT;
    RxGpioPin[1] = HW_UART_P_RX_PIN;
    TxGpioPort[1] = HW_UART_P_TX_PORT;
    TxGpioPin[1] = HW_UART_P_TX_PIN;
    ...
    RxGpioPort[2] = HW_UART_3_RX_PORT;
    RxGpioPin[2] = HW_UART_3_RX_PIN;
    TxGpioPort[2] = HW_UART_3_TX_PORT;
    TxGpioPin[2] = HW_UART_3_TX_PIN;
...
void app_uartcomm_start(UART_PORT port_number)
{
    ...
    packet_init(write_functions[port_number], process_functions[port_number],
                &packet_state[port_number]);
    ...
    palSetPadMode(TxGpioPort[port_number], TxGpioPin[port_number],
                  PAL_MODE_ALTERNATE(gpioAF[port_number]) |
                  PAL_STM32_OSPEED_HIGHEST | PAL_STM32_PUDR_PULLUP);
    palSetPadMode(RxGpioPort[port_number], RxGpioPin[port_number],
                  PAL_MODE_ALTERNATE(gpioAF[port_number]) |
                  PAL_STM32_OSPEED_HIGHEST | PAL_STM32_PUDR_PULLUP);
...
static THD_FUNCTION(packet_process_thread, arg)
{
    ...
    {
        chEvtRegisterMaskWithFlags(&(*serialPortDriverRx[port_number]).event, &el[port_number], EVENT_MASK(
