...
static lbm_value ext_pod_home(lbm_value* args, lbm_uint argn)
{
    (void) args;
    (void) argn;
    m_pod_state.homing_angle_now = HOMING_ANGLE_BACK + m_pod_state.req_angle +
                                   m_pod_state.angle_home + m_pod_state.angle_offset;
...
static THD_FUNCTION(control_thread, arg)
{
    ...
    for(;;)
    {
        ...
        float angle_target_no_filter = m_pod_state.req_angle + m_pod_state.angle_home +
                                       m_pod_state.angle_offset;
