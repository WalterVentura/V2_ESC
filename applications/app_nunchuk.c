...
void app_nunchuk_update_output(chuck_data* data)
{
    if(!output_running)
    {
        ...
        output_running = true;
        chuck_d.js_y = 128;
        chThdCreateStatic(output_thread_wa, sizeof(output_thread_wa), NORMALPRIO, output_thread,
                          NULL);
    }
...
static THD_FUNCTION(output_thread, arg)
{
    ...
    for(;;)
    {
        ...
        out_val = utils_throttle_curve(out_val, config.throttle_exp, config.throttle_exp_brake,
        ...
        if(chuck_d.bt_c)
        {
            ...
            else
            {
                if(is_reverse)
                {
                    ...
                    pid_rpm -= (out_val * config.stick_erpm_per_s_in_cc) *
                               ((float) OUTPUT_ITERATION_TIME_MS / 1000.0);
        ...
        if(config.use_smart_rev && (config.ctrl_type != CHUK_CTRL_TYPE_CURRENT_BIDIRECTIONAL))
        {
            ...
            if((out_val < -0.92) && (duty_highest_abs < (mcconf->l_min_duty * 1.5)) &&
               (fabsf(current_highest) <
                (mcconf->l_current_max * mcconf->l_current_max_scale * 0.7)))
            {
                duty_control = true;
            }
        ...
        float ramp_time = fabsf(current) >
                          fabsf(prev_current) ? config.ramp_time_pos : config.ramp_time_neg;

        if(ramp_time > 0.01)
        {
            const float ramp_step = ((float) OUTPUT_ITERATION_TIME_MS * current_range) /
                                    (ramp_time * 1000.0);
        ...
        else
        {
            ...
            if(config.multi_esc)
            {
                for(int i = 0; i < CAN_STATUS_MSGS_TO_STORE; i++)
                {
                    ...
                    if((msg->id >= 0) && (UTILS_AGE_S(msg->rx_time) < MAX_CAN_AGE))
                    {
                        bool is_braking = (current > 0.0 && msg->duty < 0.0) ||
                                          (current < 0.0 && msg->duty > 0.0);
                ...
                bool is_braking = (current > 0.0 && duty_now < 0.0) ||
                                  (current < 0.0 && duty_now > 0.0);
