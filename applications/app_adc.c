...
static THD_FUNCTION(adc_thread, arg)
{
    ...
    for(;;)
    {
        ...
        pwr = utils_throttle_curve(pwr, config.throttle_exp, config.throttle_exp_brake,
                                   config.throttle_exp_mode);

        // Apply ramping
        static systime_t last_time = 0;
        static float pwr_ramp = 0.0;
        float ramp_time = fabsf(pwr) >
                          fabsf(pwr_ramp) ? config.ramp_time_pos : config.ramp_time_neg;
        ...
        switch(config.ctrl_type)
        {
            ...
            case ADC_CTRL_TYPE_DUTY_REV_BUTTON:
                ...
                {
                    mc_interface_set_duty(utils_map(pwr, -1.0, 1.0, -mcconf->l_max_duty,
                                                    mcconf->l_max_duty));
                    send_duty = true;
                }
                    ...
                    for(int i = 0; i < CAN_STATUS_MSGS_TO_STORE; i++)
                    {
                        ...
                        if((msg->id >= 0) && (UTILS_AGE_S(msg->rx_time) < MAX_CAN_AGE))
                        {
                            if(config.tc && (config.tc_max_diff > 1.0))
                            {
                                ...
                                current_out = utils_map(diff, 0.0, config.tc_max_diff, current_rel,
                                                        0.0);
                            }
