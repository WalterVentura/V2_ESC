...
int meters_to_tac_steps(float meters)
{
    ...
    {
        commands_printf(
            "Skypuff: -- ZERO STEPS WARNING -- meters_to_tac_steps(%.5f) results in %d steps",
            (double) meters, steps);
    }
...
inline static void brake(int cur_tac)
{
    if(state != BRAKING)
    {
        prev_print = i;
        commands_printf(
            "Skypuff: state %s -> braking, position: %.2f meters (%d steps), breaking: %.1fKg (%.1fA)",
            state_str(),
            (double) tac_steps_to_meters(
                cur_tac), cur_tac,
            (double) (config.brake_current / config.kg_to_amps),
...
static void unwinding(int cur_tac)
{
    ...
    prev_print = i;
    commands_printf(
        "Skypuff: state %s -> unwinding, position: %.2fm (%d steps), pulling: %.1fKg (%.1fA)",
        state_str(),
...
static void rewinding(int cur_tac)
{
    ...
    prev_print = i;
    commands_printf(
        "Skypuff: state %s -> rewinding, position: %.2fm (%d steps), pulling: %.1fKg (%.1fA)",
        state_str(),
        (double) tac_steps_to_meters(cur_tac), cur_tac,
        (double) (current / config.kg_to_amps), (double) current);
...
static void slowing(int cur_tac, float erpm)
{
    prev_print = i;
    commands_printf(
        "Skypuff: state %s -> slowing, position: %.2fm (%d steps), speed: %.1fms (%.0f ERPM)",
        state_str(),
        (double) tac_steps_to_meters(cur_tac), cur_tac,
...
static void slow(int cur_tac, float erpm)
{
    ...
    prev_print = i;
    commands_printf(
        "Skypuff: state %s -> slow, position: %.2fm (%d steps), speed: %.1fms (%.0f ERPM), constant speed: %.1fms (%.0f ERPM)",
        state_str(), (double) tac_steps_to_meters(cur_tac), cur_tac,
        (double) erpm_to_ms(erpm), (double) erpm, (double) erpm_to_ms(constant_erpm),
        (double) constant_erpm);
        ...
        switch(state)
        {
            ...
            case UNWINDING:
                ...
                if(abs_tac > prev_abs_tac)
                {
                    ...
                    {
                        commands_printf(
                            "Skypuff: unwinded from slowing zone %.2f meters (%d steps)",
                            (double) tac_steps_to_meters(cur_tac), cur_tac);
                    }
            ...
            case SLOWING:
                ...
                if(i - prev_print > short_print_delay)
                {
                    ...
                    commands_printf(
                        ...
                        (double) erpm_to_ms(config.slow_erpm),
            ...
            case SLOW:
                ...
                if(abs_current > config.slow_max_current)
                {
                    commands_printf(
                        ...
                        (double) (config.slow_max_current / config.kg_to_amps),
                ...
                if(i - prev_print > long_print_delay)
                {
                    prev_print = i;
                    commands_printf(
                        "Skypuff: slow, position %.2f meters (%d steps), speed: %.1fms (%.0f ERPM), pulling: %.1fKg (%.1fA)",
                        (double) tac_steps_to_meters(cur_tac), cur_tac,
                        (double) erpm_to_ms(cur_erpm), (double) cur_erpm,
                        (double) (cur_current / config.kg_to_amps),
                        (double) cur_current);
                }
...
static void terminal_move_tac(int argc, const char** argv)
{
    if(argc == 2)
    {
        ...
        commands_printf(
            "move_tac: moving zero %.2fm (%d steps) %s, cur pos: %.2fm (%d steps), new pos: %.2fm (%d steps)",
    ...
    {
        commands_printf(
            "This command requires one argument: 'move_tac -5.2' will move zero 5.2 meters backward");
    }
}
...
static void terminal_show_skypuff_conf(int argc, const char** argv)
{
    ...
    commands_printf("  state %s, current position: %.2f meters (%d steps)",
                    state_str(), (double) tac_steps_to_meters(cur_tac), cur_tac);
    commands_printf("  amperes per 1kg force: %.1fAKg", (double) config.kg_to_amps);
    commands_printf("  braking range: %.2f meters (%d steps)",
                    (double) tac_steps_to_meters(config.braking_length), config.braking_length);
    commands_printf("  overwinding: %.2f meters (%d steps)",
                    (double) tac_steps_to_meters(config.overwinding), config.overwinding);
    commands_printf("  slowing range: %.2f meters (%d steps)",
                    (double) tac_steps_to_meters(config.slowing_length), config.slowing_length);
    commands_printf("  rewinding trigger range: %.2f meters (%d steps)",
                    (double) tac_steps_to_meters(
                        config.rewinding_trigger_lenght), config.rewinding_trigger_lenght);
    commands_printf("  unwinding trigger range: %.2f meters (%d steps)",
                    (double) tac_steps_to_meters(
                        config.unwinding_trigger_length), config.unwinding_trigger_length);
    commands_printf("  brake force: %.2fkg (%.1fA)",
                    (double) (config.brake_current / config.kg_to_amps),
                    (double) config.brake_current);
    commands_printf("  unwinding force: %.2fkg (%.1fA)",
                    (double) (config.unwinding_current / config.kg_to_amps),
                    (double) config.unwinding_current);
    commands_printf("  rewinding force: %.2fkg (%.1fA)",
                    (double) (config.rewinding_current / config.kg_to_amps),
    ...
    commands_printf("  maximum slow force: %.2fkg (%.1fA)",
                    (double) (config.slow_max_current / config.kg_to_amps),
                    (double) config.slow_max_current);
}
