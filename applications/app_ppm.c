    ...
    for(;;)
    {
        ...
        if(timeout_has_timeout() || (servodec_get_time_since_update() > timeout_get_timeout_msec()))
        {
            ...
            continue;
                ...
                (config.safe_start != SAFE_START_NO_FAULT))
        {
            pulses_without_power = 0;
        }
        ...
        servo_val = utils_throttle_curve(servo_val, config.throttle_exp, config.throttle_exp_brake,
                                         config.throttle_exp_mode);

        // Apply ramping
        static systime_t last_time = 0;
        static float servo_val_ramp = 0.0;
        float ramp_time = fabsf(servo_val) >
                          fabsf(servo_val_ramp) ? config.ramp_time_pos : config.ramp_time_neg;
...
// }
        ...
        switch(config.ctrl_type)
        {
            ...
            case PPM_CTRL_TYPE_DUTY_NOREV:
                ...
                {
                    mc_interface_set_duty(utils_map(servo_val, -1.0, 1.0, -mcconf->l_max_duty,
                                                    mcconf->l_max_duty));
                    send_duty = true;
                }
                    ...
                    for(int i = 0; i < CAN_STATUS_MSGS_TO_STORE; i++)
                    {
                        ...
                        if((msg->id >= 0) && (UTILS_AGE_S(msg->rx_time) < MAX_CAN_AGE))
                        {
                            ...
                            if(config.tc && (config.tc_max_diff > 1.0) && !autoTCdisengaged)
                            {
                                ...
                                servo_val =
                                    utils_map(diff, 0.0, config.tc_max_diff, servo_val, 0.0);
                            }
