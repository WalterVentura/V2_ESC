...
static THD_FUNCTION(my_thread, arg)
{
    ...
    for(;;)
    {
        ...
        if(io_v1 && io_v2 && (UTILS_AGE_S(io_v1->rx_time) < 1.0) &&
           (UTILS_AGE_S(io_v2->rx_time) < 1.0))
        {
        ...
        float app_adc_pwr = 0.0;
        {
            ...
            float ramp_time = fabsf(app_adc_pwr) >
                              fabsf(pwr_ramp) ? app_adc_config.ramp_time_pos : app_adc_config.
                                  ramp_time_neg;
        ...
        {
            pedal_current_target = utils_map(erpm_motor, 0.0, erpm_ramp_in, 0.0,
                                             pedal_current_target);
        }
