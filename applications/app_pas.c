    ...
    for(;;)
    {
        ...
        switch(config.ctrl_type)
        {
            ...
            case PAS_CTRL_TYPE_CADENCE:
                ...
                if(config.pedal_rpm_end > (config.pedal_rpm_start + 1.0))
                {
                    output = utils_map(pedal_rpm, config.pedal_rpm_start, config.pedal_rpm_end, 0.0,
                                       config.current_scaling * sub_scaling);
            ...
            case PAS_CTRL_TYPE_TORQUE_WITH_CADENCE_TIMEOUT:
            {
                ...
                {
                    ms_without_cadence_or_torque += (1000.0 * (float) sleep_time) /
        ...
        float ramp_time = fabsf(output) >
                          fabsf(output_ramp) ? config.ramp_time_pos : config.ramp_time_neg;
