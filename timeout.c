...
void timeout_init(void)
{
    ...
    chThdCreateStatic(timeout_thread_wa, sizeof(timeout_thread_wa), NORMALPRIO, timeout_thread,
                      NULL);
}
...
static THD_FUNCTION(timeout_thread, arg)
{
    ...
    for(;;)
    {
        ...
        if(kill_sw ||
           ((timeout_msec != 0) && (chVTTimeElapsedSinceX(last_update_time) > MS2ST(timeout_msec))))
        {
