...
void conf_custom_add_config(int (* get_cfg)(uint8_t* data, bool is_default), bool (* set_cfg)(
                                uint8_t* data), int (* get_cfg_xml)(uint8_t** data))
{
...
void conf_custom_process_cmd(unsigned char* data, unsigned int len, void (* reply_func)(
    ...
    switch(packet_id)
    {
        ...
        case COMM_GET_CUSTOM_CONFIG_DEFAULT:
        {
            ...
            if(m_get_cfg && (conf_ind == 0))
            {
                ...
                int32_t len_cfg = m_get_cfg(send_buffer + ind,
                                            packet_id == COMM_GET_CUSTOM_CONFIG_DEFAULT);
                ind += len_cfg;
