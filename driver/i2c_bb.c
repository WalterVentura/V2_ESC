...
static inline float rate2secs(i2c_bb_state* s)
{
    switch(s->rate)
    {
        case I2C_BB_RATE_100K:
            return 3.5e-6;

        case I2C_BB_RATE_200K:
            return 1.0e-6;

        case I2C_BB_RATE_400K:
            return 2.5e-7;

        case I2C_BB_RATE_700K:
            return 0.0;
    }
...
bool i2c_bb_tx_rx(i2c_bb_state* s, uint16_t addr, uint8_t* txbuf, size_t txbytes, uint8_t* rxbuf,
                  size_t rxbytes)
{
    chMtxLock(&s->mutex);
