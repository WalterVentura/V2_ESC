...
static lbm_value ext_str_from_n(lbm_value* args, lbm_uint argn)
{
    ...
    switch(lbm_type_of(args[0]))
    {
        case LBM_TYPE_FLOAT:
            ...
            len =
                (size_t) snprintf(buffer, sizeof(buffer), format,
                                  (double) lbm_dec_as_float(args[0]));
            break;
...
static lbm_value ext_str_merge(lbm_value* args, lbm_uint argn)
{
    ...
    if(lbm_create_array(&res, len_tot + 1))
    {
        ...
        {
            offset +=
                (unsigned int) sprintf((char*) arr->data + offset, "%s", lbm_dec_str(args[i]));
        }
...
static lbm_value to_str(char* delimiter, lbm_value* args, lbm_uint argn)
{
    ...
    for(lbm_uint i = 0; i < argn; i++)
    {
        ...
        else
        {
            ...
            {
                chars = snprintf(str + str_ofs, (unsigned int) max, "%s%s", delimiter,
                                 print_val_buffer);
            }
        }
