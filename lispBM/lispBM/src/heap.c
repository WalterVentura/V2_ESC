...
lbm_value lbm_enc_i32(int32_t x)
{
    ...
    if(lbm_type_of(i) == LBM_TYPE_SYMBOL)
    {
        return i;
    }
...
lbm_value lbm_enc_u32(uint32_t x)
{
    ...
    if(lbm_type_of(u) == LBM_TYPE_SYMBOL)
    {
        return u;
    }
...
lbm_value lbm_enc_float(float x)
{
    ...
    if(lbm_type_of(f) == LBM_TYPE_SYMBOL)
    {
        return f;
    }
...
static void heap_init_state(lbm_cons_t* addr, lbm_uint num_cells, lbm_uint* gc_stack_storage,
...
int lbm_heap_allocate_array(lbm_value* res, lbm_uint size)
{
    ...
    array =
        (lbm_array_header_t*) lbm_memory_allocate(sizeof(lbm_array_header_t) / sizeof(lbm_uint));
