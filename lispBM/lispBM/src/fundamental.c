...
#define PROMOTE_SWAP(t, a, b)                                          \
    if(lbm_type_of_functional(b) == LBM_TYPE_FLOAT &&                  \
       (lbm_type_of_functional(a) < LBM_TYPE_DOUBLE)) {                \
        lbm_value tmp = a;                                             \
        a = b;                                                         \
        b = tmp;                                                       \
    } if(lbm_type_of_functional(a) == LBM_TYPE_FLOAT &&                \
         (lbm_type_of_functional(b) < LBM_TYPE_DOUBLE)) {              \
        /* DO NOTHING */                                               \
...
static lbm_uint add2(lbm_uint a, lbm_uint b)
{
    ...
    {
        case LBM_TYPE_BYTE:
            retval = lbm_enc_char((uint8_t) (lbm_dec_char(a) + lbm_dec_char(b)));
            break;

        case LBM_TYPE_I:
            retval = lbm_enc_i(lbm_dec_i(a) + lbm_dec_as_i32(b));
            break;

        case LBM_TYPE_U:
            retval = lbm_enc_u(lbm_dec_u(a) + lbm_dec_as_u32(b));
            break;

        case LBM_TYPE_U32:
            retval = lbm_enc_u32(lbm_dec_u32(a) + lbm_dec_as_u32(b));
            break;

        case LBM_TYPE_I32:
            retval = lbm_enc_i32(lbm_dec_i32(a) + lbm_dec_as_i32(b));
            break;

        case LBM_TYPE_FLOAT:
            retval = lbm_enc_float(lbm_dec_float(a) + lbm_dec_as_float(b));
            break;

        case LBM_TYPE_U64:
            retval = lbm_enc_u64(lbm_dec_u64(a) + lbm_dec_as_u64(b));
            break;

        case LBM_TYPE_I64:
            retval = lbm_enc_i64(lbm_dec_i64(a) + lbm_dec_as_i64(b));
            break;

        case LBM_TYPE_DOUBLE:
            retval = lbm_enc_double(lbm_dec_double(a) + lbm_dec_as_double(b));
            break;
    }
...
static lbm_uint div2(lbm_uint a, lbm_uint b)
{
    ...
    lbm_uint t =
        (lbm_type_of_functional(a) <
         lbm_type_of_functional(b)) ? lbm_type_of_functional(b) : lbm_type_of_functional(a);
    ...
    switch(t)
    {
        ...
        case LBM_TYPE_I:
            ...
            retval = lbm_enc_i(lbm_dec_as_i32(a) / lbm_dec_as_i32(b));
            break;

        case LBM_TYPE_U:

            if(lbm_dec_as_u32(b) == 0)
            {
                return ENC_SYM_DIVZERO;
            }

            retval = lbm_enc_u(lbm_dec_as_u32(a) / lbm_dec_as_u32(b));
            break;
        ...
        case LBM_TYPE_U32:
            ...
            retval = lbm_enc_u32(lbm_dec_as_u32(a) / lbm_dec_as_u32(b));
            break;

        case LBM_TYPE_I32:

            if(lbm_dec_as_i32(b) == 0)
            {
                return ENC_SYM_DIVZERO;
            }

            retval = lbm_enc_i32(lbm_dec_as_i32(a) / lbm_dec_as_i32(b));
            break;

        case LBM_TYPE_FLOAT:

            if((lbm_dec_as_float(b) == 0.0f) || (lbm_dec_as_float(b) == -0.0f))
            {
                return ENC_SYM_DIVZERO;
            }

            retval = lbm_enc_float(lbm_dec_as_float(a) / lbm_dec_as_float(b));
            break;

        case LBM_TYPE_U64:

            if(lbm_dec_as_u64(b) == 0)
            {
                return ENC_SYM_DIVZERO;
            }

            retval = lbm_enc_u64(lbm_dec_as_u32(a) / lbm_dec_as_u64(b));
            break;

        case LBM_TYPE_I64:

            if(lbm_dec_as_i64(b) == 0)
            {
                return ENC_SYM_DIVZERO;
            }

            retval = lbm_enc_i64(lbm_dec_as_i32(a) / lbm_dec_as_i64(b));
            break;

        case LBM_TYPE_DOUBLE:

            if((lbm_dec_as_double(b) == (double) 0.0) || (lbm_dec_as_double(b) == (double) -0.0))
            {
                return ENC_SYM_DIVZERO;
            }

            retval = lbm_enc_double(lbm_dec_as_double(a) / lbm_dec_as_double(b));
            break;
    }
...
static lbm_uint mod2(lbm_uint a, lbm_uint b)
{
    ...
    switch(t)
    {
        ...
        case LBM_TYPE_FLOAT:
            ...
            retval = lbm_enc_float(fmodf(lbm_dec_as_float(a), lbm_dec_as_float(b)));
            break;
        ...
        case LBM_TYPE_U64:
            ...
            retval = lbm_enc_u64(lbm_dec_as_u64(a) % lbm_dec_as_u64(b));
            break;
        ...
        case LBM_TYPE_I64:
            ...
            retval = lbm_enc_i64(lbm_dec_as_i64(a) % lbm_dec_as_i64(b));
            break;
        ...
        case LBM_TYPE_DOUBLE:
            ...
            retval = lbm_enc_double(fmod(lbm_dec_as_double(a), lbm_dec_as_double(b)));
            break;
    }
...
static lbm_uint negate(lbm_uint a)
{
    ...
    {
        case LBM_TYPE_BYTE:
            retval = lbm_enc_char((uint8_t) (256 - (int) (lbm_dec_char(a))));
            break;

        case LBM_TYPE_I:
            retval = lbm_enc_i(-lbm_dec_i(a));
            break;

        case LBM_TYPE_U:
            retval = lbm_enc_u(-lbm_dec_u(a));
            break;

        case LBM_TYPE_U32:
            retval = lbm_enc_u32(-lbm_dec_u32(a));
            break;

        case LBM_TYPE_I32:
            retval = lbm_enc_i32(-lbm_dec_i32(a));
            break;

        case LBM_TYPE_FLOAT:
            retval = lbm_enc_float(-lbm_dec_float(a));
            break;

        case LBM_TYPE_U64:
            retval = lbm_enc_u64(-lbm_dec_u64(a));
            break;

        case LBM_TYPE_I64:
            retval = lbm_enc_i64(-lbm_dec_i64(a));
            break;

        case LBM_TYPE_DOUBLE:
            retval = lbm_enc_double(-lbm_dec_double(a));
            break;
    }
...
static int compare(lbm_uint a, lbm_uint b)
{
    ...
    {
        case LBM_TYPE_CHAR:
            retval = CMP(lbm_dec_char(a), lbm_dec_char(b));
            break;

        case LBM_TYPE_I:
            retval = CMP(lbm_dec_as_i32(a), lbm_dec_as_i32(b));
            break;

        case LBM_TYPE_U:
            retval = CMP(lbm_dec_as_u32(a), lbm_dec_as_u32(b));
            break;
        ...
        case LBM_TYPE_FLOAT:
            retval = CMP(lbm_dec_as_float(a), lbm_dec_as_float(b));
            break;

        case LBM_TYPE_U64:
            retval = CMP(lbm_dec_as_u64(a), lbm_dec_as_u64(b));
            break;

        case LBM_TYPE_I64:
            retval = CMP(lbm_dec_as_i64(a), lbm_dec_as_i64(b));
            break;

        case LBM_TYPE_DOUBLE:
            retval = CMP(lbm_dec_as_double(a), lbm_dec_as_double(b));
            break;
    }
...
static lbm_value fundamental_shl(lbm_value* args, lbm_uint nargs, eval_context_t* ctx)
{
    ...
    if(nargs == 2)
    {
        ...
        {
            case LBM_TYPE_I:
                retval = lbm_enc_i(lbm_dec_i(args[0]) << lbm_dec_as_u32(args[1]));
                break;

            case LBM_TYPE_U:
                retval = lbm_enc_u(lbm_dec_u(args[0]) << lbm_dec_as_u32(args[1]));
                break;

            case LBM_TYPE_U32:
                retval = lbm_enc_u32(lbm_dec_u32(args[0]) << lbm_dec_as_u32(args[1]));
                break;

            case LBM_TYPE_I32:
                retval = lbm_enc_i32(lbm_dec_i32(args[0]) << lbm_dec_as_u32(args[1]));
                break;

            case LBM_TYPE_I64:
                retval = lbm_enc_i64(lbm_dec_i64(args[0]) << lbm_dec_as_u32(args[1]));
                break;

            case LBM_TYPE_U64:
                retval = lbm_enc_u64(lbm_dec_u64(args[0]) << lbm_dec_as_u32(args[1]));
                break;
        }
    }
...
static lbm_value fundamental_bitwise_and(lbm_value* args, lbm_uint nargs, eval_context_t* ctx)
{
    ...
    if(nargs == 2)
    {
        ...
        {
            case LBM_TYPE_I:
                retval = lbm_enc_i(lbm_dec_i(args[0]) & lbm_dec_as_i32(args[1]));
                break;
            ...
            case LBM_TYPE_I32:
                retval = lbm_enc_i32(lbm_dec_i32(args[0]) & lbm_dec_as_i32(args[1]));
                break;

            case LBM_TYPE_I64:
                retval = lbm_enc_i64(lbm_dec_i64(args[0]) & lbm_dec_as_i64(args[1]));
                break;

            case LBM_TYPE_U64:
                retval = lbm_enc_u64(lbm_dec_u64(args[0]) & lbm_dec_as_u64(args[1]));
                break;
        }
    }
...
static lbm_value fundamental_bitwise_not(lbm_value* args, lbm_uint nargs, eval_context_t* ctx)
{
    ...
    if(nargs == 1)
    {
        ...
        {
            case LBM_TYPE_I:
                retval = lbm_enc_i(~lbm_dec_i(args[0]));
                break;

            case LBM_TYPE_U:
                retval = lbm_enc_u(~lbm_dec_u(args[0]));
                break;

            case LBM_TYPE_U32:
                retval = lbm_enc_u32(~lbm_dec_u32(args[0]));
                break;

            case LBM_TYPE_I32:
                retval = lbm_enc_i32(~lbm_dec_i32(args[0]));
                break;

            case LBM_TYPE_I64:
                retval = lbm_enc_i64(~lbm_dec_i64(args[0]));
                break;

            case LBM_TYPE_U64:
                retval = lbm_enc_u64(~lbm_dec_u64(args[0]));
                break;
        }
    }
...
static lbm_value fundamental_type_of(lbm_value* args, lbm_uint nargs, eval_context_t* ctx)
{
    ...
    {
        case LBM_TYPE_CONS:
            return ENC_SYM_TYPE_LIST;

        case LBM_TYPE_ARRAY:
            return ENC_SYM_TYPE_ARRAY;

        case LBM_TYPE_I32:
            return ENC_SYM_TYPE_I32;

        case LBM_TYPE_U32:
            return ENC_SYM_TYPE_U32;

        case LBM_TYPE_FLOAT:
            return ENC_SYM_TYPE_FLOAT;

        case LBM_TYPE_I64:
            return ENC_SYM_TYPE_I64;

        case LBM_TYPE_U64:
            return ENC_SYM_TYPE_U64;

        case LBM_TYPE_DOUBLE:
            return ENC_SYM_TYPE_DOUBLE;

        case LBM_TYPE_I:
            return ENC_SYM_TYPE_I;

        case LBM_TYPE_U:
            return ENC_SYM_TYPE_U;

        case LBM_TYPE_CHAR:
            return ENC_SYM_TYPE_CHAR;

        case LBM_TYPE_SYMBOL:
            return ENC_SYM_TYPE_SYMBOL;
    }
