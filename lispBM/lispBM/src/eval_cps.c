...
void lbm_set_critical_error_callback(void (* fptr)(void))
{
    if(fptr == NULL)
    {
        critical_error_callback = critical_nonsense;
    }
    else
    {
        critical_error_callback = fptr;
    }
}
...
void lbm_set_usleep_callback(void (* fptr)(uint32_t))
{
    if(fptr == NULL)
    {
        usleep_callback = usleep_nonsense;
    }
    else
    {
...
void lbm_set_timestamp_us_callback(uint32_t (* fptr)(void))
{
    if(fptr == NULL)
    {
        timestamp_us_callback = timestamp_nonsense;
    }
    else
    {
...
void lbm_set_ctx_done_callback(void (* fptr)(eval_context_t*))
{
    if(fptr == NULL)
    {
        ctx_done_callback = ctx_done_nonsense;
    }
    else
    {
...
void lbm_set_printf_callback(int (* fptr)(const char*, ...))
{
    if(fptr == NULL)
    {
        printf_callback = printf_nonsense;
    }
    else
    {
...
void lbm_set_dynamic_load_callback(bool (* fptr)(const char*, const char**))
{
    if(fptr == NULL)
    {
        dynamic_load_callback = dynamic_load_nonsense;
    }
    else
    {
...
static bool event_internal(lbm_event_type_t event_type, lbm_uint parameter, lbm_uint buf_ptr,
                           lbm_uint buf_len)
{
    bool r = false;
...
static inline void stack_push_5(lbm_stack_t* s, lbm_uint v1, lbm_uint v2, lbm_uint v3, lbm_uint v4,
                                lbm_uint v5)
{
    if(s->sp + 4 < s->size)
    {
...
static inline void stack_push_6(lbm_stack_t* s, lbm_uint v1, lbm_uint v2, lbm_uint v3, lbm_uint v4,
                                lbm_uint v5, lbm_uint v6)
{
    if(s->sp + 5 < s->size)
    {
        lbm_uint* t = &s->data[s->sp];
...
static void call_fundamental(lbm_uint fundamental, lbm_value* args, lbm_uint arg_count,
                             eval_context_t* ctx)
{
    lbm_value res;
...
void print_error_message(lbm_value error, bool has_at, lbm_value at, unsigned int row,
                         unsigned int col, lbm_int row0, lbm_int row1)
{
    if(!printf_callback)
    {
        return;
...
bool create_string_channel(char* str, lbm_value* res)
{
    ...
    st = (lbm_string_channel_state_t*) lbm_memory_allocate(
        sizeof(lbm_string_channel_state_t) / sizeof(lbm_uint) + 1);
    ...
    chan = (lbm_char_channel_t*) lbm_memory_allocate(
        sizeof(lbm_char_channel_t) / sizeof(lbm_uint) + 1);
    ...
    lbm_value cell =
        lbm_heap_allocate_cell(LBM_TYPE_CHANNEL, (lbm_uint) chan, ENC_SYM_CHANNEL_TYPE);
...
static void error_ctx_base(lbm_value err_val, bool has_at, lbm_value at, unsigned int row,
                           unsigned int column)
{
...
static lbm_cid lbm_create_ctx_parent(lbm_value program, lbm_value env, lbm_uint stack_size,
                                     lbm_cid parent, uint32_t context_flags, char* name)
{
    if(!lbm_is_cons(program))
    {
        return -1;
    }
...
static void eval_cond(eval_context_t* ctx)
{
    ...
    else
    {
        ...
        WITH_GC(rest, lbm_heap_allocate_list_init(2,
                                                  body, // Then branch
                                                  cons_with_gc(ENC_SYM_COND,
                                                               get_cddr(
                                                                   ctx->curr_exp), ENC_SYM_NIL)));
...
static void apply_spawn_base(lbm_value* args, lbm_uint nargs, eval_context_t* ctx,
...
static void apply_merge(lbm_value* args, lbm_uint nargs, eval_context_t* ctx)
{
    if((nargs == 3) && lbm_is_list(args[1]) && lbm_is_list(args[2]))
    {
        if(!lbm_is_closure(args[0]))
        {
            ...
            lbm_value body;
            WITH_GC_RMBR_1(body, lbm_heap_allocate_list_init(3, args[0], symbol_x,
                                                             symbol_y), closure);
    ...
    switch(fun_kind)
    {
        case SYMBOL_KIND_EXTENSION:
        {
            ...
            if(blocking_extension)
            {
                ...
                {
                    blocking_extension_timeout = false;
                    block_current_ctx(LBM_THREAD_STATE_TIMEOUT, blocking_extension_timeout_us,
                                      true);
                }
                else
                {
...
static void cont_read_next_token(eval_context_t* ctx)
{
    ...
    if(n > 0)
    {
        ...
        switch(match)
        {
            ...
            case TOKCONSTSYMSTR:
                ...
                if(match == TOKCONSTSTART)
                {
                    ctx->flags |= EVAL_CPS_CONTEXT_FLAG_CONST;
                }

                if(match == TOKCONSTEND)
                {
                    ctx->flags &= ~EVAL_CPS_CONTEXT_FLAG_CONST;
                }

                if(match == TOKCONSTSYMSTR)
                {
    ...
    if(n > 0)
    {
        ...
        switch(int_result.type)
        {
            case TOKTYPEBYTE:
                res =
                    lbm_enc_char((uint8_t) (int_result.negative ? -int_result.value : int_result.
                                                value));
                break;

            case TOKTYPEI:
                res =
                    lbm_enc_i((lbm_int) (int_result.negative ? -int_result.value : int_result.value));
                break;

            case TOKTYPEU:
                res =
                    lbm_enc_u(
                        (lbm_uint) (int_result.negative ? -int_result.value : int_result.value));
                break;

            case TOKTYPEI32:
                WITH_GC(res,
                        lbm_enc_i32((int32_t) (int_result.negative ? -int_result.value : int_result.
                                                   value)));
                break;

            case TOKTYPEU32:
                WITH_GC(res,
                        lbm_enc_u32((uint32_t) (int_result.negative ? -int_result.value : int_result
                                                    .value)));
                break;

            case TOKTYPEI64:
                WITH_GC(res,
                        lbm_enc_i64((int64_t) (int_result.negative ? -int_result.value : int_result.
                                                   value)));
                break;

            case TOKTYPEU64:
                WITH_GC(res,
                        lbm_enc_u64((uint64_t) (int_result.negative ? -int_result.value : int_result
                                                    .value)));
                break;
    ...
    else if(lbm_is_cons(ctx->r))
    {
        ...
        switch(get_car(ctx->r))
        {
            case ENC_SYM_CLOSURE:
            {
                ...
                if(p_nil)
                {
                    lbm_value rest_binder =
                        allocate_binding(ENC_SYM_REST_ARGS, ENC_SYM_NIL, cl[CLO_ENV]);
                    lbm_value* reserved = stack_reserve(ctx, 4);
                    sptr[0] = arg_env;
                    sptr[1] = cl[CLO_BODY];
        ...
        if(lbm_type_of(ref->cdr) == LBM_TYPE_SYMBOL)
        {
            switch(lbm_dec_sym(ref->cdr))
            {
                ...
                case SYM_ARRAY_TYPE:
                {
                    ...
                    lbm_uint flash_arr;
                    handle_flash_status(lbm_write_const_array_padded((uint8_t*) arr->data,
                                                                     arr->size, &flash_arr));
...
{
    if(lbm_is_symbol_nil(front))
    {
        return back;
    }

    if(lbm_is_symbol_nil(back))
    {
        return front;
    }
...
void lbm_pause_eval(void)
{
    ...
    if(eval_cps_next_state != eval_cps_run_state)
    {
        eval_cps_state_changed = true;
    }
}
