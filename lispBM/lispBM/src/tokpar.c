...
static int tok_match_fixed_size_tokens(lbm_char_channel_t* chan, const matcher* m,
                                       unsigned int start_pos, unsigned int num, uint32_t* res)
{
...
int tok_double(lbm_char_channel_t* chan, token_float* result)
{
    ...
    uint32_t tok_res;
    int type_len = tok_match_fixed_size_tokens(chan, type_qual_table, n, NUM_TYPE_QUALIFIERS,
                                               &tok_res);

    if(type_len == TOKENIZER_NEED_MORE)
    {
        return type_len;
    }
