    ...
    while((c = getopt_long(argc, argv, "H:hs:", options, &opt_index)) != -1)
    {
        switch(c)
        {
            ...
            case 'h':
                ...
                printf(
                    "    --silent                      The REPL will print as little as possible\n");
                printf("\n");
...
bool load_flat_library(unsigned char* lib, unsigned int size)
{
    ...
    while(pos < (size - 1))
    {
        uint32_t name_size = read_word(lib, pos);
        pos += 4;
        ...
        uint32_t val_size = read_word(lib, pos);
        pos += 4;
    ...
    if(env_input_file)
    {
        ...
        while(true)
        {
            ...
            while(!lbm_global_env_lookup(&binding, key))
            {
                ...
                if(count > 10000)
                {
                    terminate_repl(REPL_EXIT_ENV_POPULATION_TIMEOUT);
                }
...
int store_env(char* filename)
{
    ...
    for(int i = 0; i < GLOBAL_ENV_ROOTS; i++)
    {
        ...
        while(lbm_is_cons(curr))
        {
            ...
            if(!name)
            {
                return REPL_EXIT_UNABLE_TO_ACCESS_SYMBOL_STRING;
            }
...
int main(int argc, char** argv)
{
    ...
    while(1)
    {
        ...
        else if(strncmp(str, ":prof report", 12) == 0)
        {
            ...
            printf("System:\t%u\t%f%%\n", num_system,
                   100.0 * ((float) num_system / (float) tot_samples));
            printf("Sleep:\t%u\t%f%%\n", num_sleep,
                   100.0 * ((float) num_sleep / (float) tot_samples));
