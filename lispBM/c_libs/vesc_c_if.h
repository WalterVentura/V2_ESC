...
typedef struct
{
    ...
    void (* can_set_current_rel_off_delay)(uint8_t controller_id, float current_rel,
                                           float off_delay);
    ...
    void (* packet_init)(void (* s_func)(unsigned char*, unsigned int),
                         void (* p_func)(unsigned char*, unsigned int), PACKET_STATE_t*);
    ...
    // Custom config
    void (* conf_custom_add_config)(int (* get_cfg)(uint8_t* data, bool is_default),
    ...
    void (* commands_unregister_reply_func)(void (* reply_func)(unsigned char* data,
                                                                unsigned int   len));
    ...
    void (* ahrs_update_madgwick_imu)(float* gyroXYZ, float* accelXYZ, float dt,
                                      ATTITUDE_INFO* att);
    ...
    void (* encoder_set_custom_callbacks)(float (* read_deg)(void), bool (* has_fault)(void),
    ...
    bool (* foc_play_audio_samples)(const int8_t* samples, int num_samp, float f_samp,
                                    float voltage);
} vesc_c_if;
...
#define HEADER              static volatile int __attribute__((__section__(".program_ptr"))) \
    prog_ptr;
