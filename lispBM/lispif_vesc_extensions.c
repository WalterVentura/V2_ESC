...
static lbm_value ext_foc_play_tone(lbm_value* args, lbm_uint argn)
{
    LBM_CHECK_ARGN_NUMBER(3);
    timeout_reset();
    bool res = mcpwm_foc_play_tone(lbm_dec_as_float(args[0]), lbm_dec_as_float(
                                       args[1]), lbm_dec_as_float(args[2]));
...
{
    if((argn != 3) || !lbm_is_array_r(args[0]) || !lbm_is_number(args[1]) ||
...
static lbm_value ext_pos_pid_error(lbm_value* args, lbm_uint argn)
{
    (void) args;
    (void) argn;
    return lbm_enc_float(utils_angle_difference(mc_interface_get_pid_pos_set(),
...
static lbm_value ext_observer_error(lbm_value* args, lbm_uint argn)
{
    (void) args;
    (void) argn;
    return lbm_enc_float(utils_angle_difference(mcpwm_foc_get_phase_observer(),
...
static lbm_value ext_can_current(lbm_value* args, lbm_uint argn)
{
    ...
    {
        comm_can_set_current_off_delay(lbm_dec_as_i32(args[0]), lbm_dec_as_float(
                                           args[1]), lbm_dec_as_float(args[2]));
    }
    else
    {
...
static lbm_value ext_can_current_rel(lbm_value* args, lbm_uint argn)
{
    ...
    {
        comm_can_set_current_rel_off_delay(lbm_dec_as_i32(args[0]), lbm_dec_as_float(
                                               args[1]), lbm_dec_as_float(args[2]));
    }
    else
    {
...
static lbm_value ext_raw_adc_current(lbm_value* args, lbm_uint argn)
{
    ...
    if(motor == 2)
    {
        scale1 = FAC_CURRENT1_M2;
        scale2 = FAC_CURRENT2_M2;
        scale3 = FAC_CURRENT3_M2;
    }
    else
    {
        scale1 = FAC_CURRENT1;
    ...
    {
        case 1:
            return lbm_enc_float((ph1 - ofs1) * scale1);

        case 2:
            return lbm_enc_float((ph2 - ofs2) * scale2);

        case 3:
            return lbm_enc_float((ph3 - ofs3) * scale3);
...
static void detect_task(void* arg)
{
    ...
    int res = conf_general_detect_apply_all_foc_can(a->detect_can, a->max_power_loss,
                                                    a->min_current_in, a->max_current_in,
                                                    a->openloop_rpm, a->sl_erpm, NULL);
...
static void measure_inductance_task(void* arg)
{
    ...
    {
        mc_interface_select_motor_thread(a->motor);
        fault = mcpwm_foc_measure_inductance_current(a->current, a->samples,
                                                     &real_measurement_current, &ld_lq_diff,
                                                     &ld_lq_avg);
        mc_interface_select_motor_thread(1);
    ...
    return make_list(3,
                     ...
                     make_list(1,
                               make_list(2,
                                         ...
                                         make_list(3,
                                                   lbm_enc_sym(SYM_LAMBDA),
                                                   make_list(3, it, lbm_enc_sym(sym_res),
                                                             lbm_enc_sym(sym_break)),
                                                   make_list(4,
                                                             lbm_enc_sym(SYM_IF),
                                                             cond,
                                                             make_list(4, lbm_enc_sym(sym_loop),
                                                                       update, body,
                                                                       lbm_enc_sym(sym_break)),
                                                             lbm_enc_sym(sym_res))))),
                     make_list(2,
                               ...
                               make_list(3,
                                         ...
                                         make_list(4, lbm_enc_sym(sym_loop), start, ENC_SYM_NIL,
                                                   lbm_enc_sym(sym_brk)))));
}
    ...
    return make_list(3,
                     ...
                     make_list(1,
                               make_list(2,
                                         ...
                                         make_list(3,
                                                   lbm_enc_sym(SYM_LAMBDA),
                                                   make_list(2, lbm_enc_sym(sym_res),
                                                             lbm_enc_sym(sym_break)),
                                                   make_list(4,
                                                             lbm_enc_sym(SYM_IF),
                                                             cond,
                                                             make_list(3, lbm_enc_sym(sym_loop),
                                                                       body,
                                                                       lbm_enc_sym(sym_break)),
                                                             lbm_enc_sym(sym_res))))),
                     make_list(2,
                               ...
                               make_list(3,
                                         lbm_enc_sym(SYM_LAMBDA),
                                         make_list(1, lbm_enc_sym(sym_brk)),
                                         make_list(3, lbm_enc_sym(sym_loop), ENC_SYM_NIL,
                                                   lbm_enc_sym(sym_brk)))));
}
                     ...
                     make_list(1,
                               make_list(2,
                                         ...
                                         make_list(3,
                                                   ...
                                                   make_list(4,
                                                             lbm_enc_sym(SYM_IF),
                                                             make_list(3, lbm_enc_sym(SYM_LT), it,
                                                                       end),
                                                             make_list(4, lbm_enc_sym(sym_loop),
                                                                       make_list(3,
                                                                                 lbm_enc_sym(SYM_ADD),
    ...
    return make_list(3,
                     ...
                     make_list(1,
                               make_list(2,
                                         ...
                                         make_list(3,
                                                   lbm_enc_sym(SYM_LAMBDA),
                                                   make_list(4, it, lbm_enc_sym(sym_rst),
                                                             lbm_enc_sym(sym_res),
                                                             lbm_enc_sym(sym_break)),
                                                   make_list(4,
                                                             lbm_enc_sym(SYM_IF),
                                                             make_list(3, lbm_enc_sym(SYM_EQ), it,
                                                                       ENC_SYM_NIL),
                                                             lbm_enc_sym(sym_res),
                                                             make_list(5,
                                                                       lbm_enc_sym(sym_loop),
...
static THD_FUNCTION(event_thread, arg)
{
    ...
    for(;;)
    {
        ...
        for(int i = 0; i < RMSG_SLOT_NUM; i++)
        {
            ...
            if((s->cid >= 0) && (s->timeout_secs > 0.0) &&
               (UTILS_AGE_S(s->start_time) > s->timeout_secs))
            {
...
void lispif_load_vesc_extensions(void)
{
    ...
    if(event_tp == NULL)
    {
        ...
        chThdCreateStatic(event_thread_wa, sizeof(event_thread_wa), NORMALPRIO - 2, event_thread,
                          NULL);
    }
