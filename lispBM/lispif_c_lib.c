...
void packet_init(void (* s_func)(unsigned char* data, unsigned int len), void (* p_func)(
    ...
    if(mem)
    {
        ...
        if(info)
        {
            ...
            if(lib_running_threads_cnt <
               (sizeof(lib_running_threads) / sizeof(lib_running_threads[0])))
            {
    ...
    *is_analog = false;
    ...
    switch(io)
    {
        ...
        case VESC_PIN_HALL4:
            ...
            *port = HW_HALL_ENC_GPIO4;
            *pin = HW_HALL_ENC_PIN4;
            res = true;
#endif
            break;
        ...
        case VESC_PIN_HALL5:
            ...
            *port = HW_HALL_ENC_GPIO5;
            *pin = HW_HALL_ENC_PIN5;
            res = true;
#endif
            break;
        ...
        case VESC_PIN_HALL6:
            ...
            *port = HW_HALL_ENC_GPIO6;
            *pin = HW_HALL_ENC_PIN6;
            res = true;
#endif
            break;
...
static float lib_get_cfg_float(CFG_PARAM p)
{
    ...
    switch(p)
    {
        case CFG_PARAM_l_current_max:
            res = mcconf->l_current_max;
            break;

        case CFG_PARAM_l_current_min:
            res = mcconf->l_current_min;
            break;

        case CFG_PARAM_l_in_current_max:
            res = mcconf->l_in_current_max;
            break;

        case CFG_PARAM_l_in_current_min:
            res = mcconf->l_in_current_min;
            break;

        case CFG_PARAM_l_abs_current_max:
            res = mcconf->l_abs_current_max;
            break;

        case CFG_PARAM_l_min_erpm:
            res = mcconf->l_min_erpm;
            break;

        case CFG_PARAM_l_max_erpm:
            res = mcconf->l_max_erpm;
            break;

        case CFG_PARAM_l_erpm_start:
            res = mcconf->l_erpm_start;
            break;

        case CFG_PARAM_l_max_erpm_fbrake:
            res = mcconf->l_max_erpm_fbrake;
            break;

        case CFG_PARAM_l_max_erpm_fbrake_cc:
            res = mcconf->l_max_erpm_fbrake_cc;
            break;

        case CFG_PARAM_l_min_vin:
            res = mcconf->l_min_vin;
            break;

        case CFG_PARAM_l_max_vin:
            res = mcconf->l_max_vin;
            break;

        case CFG_PARAM_l_battery_cut_start:
            res = mcconf->l_battery_cut_start;
            break;

        case CFG_PARAM_l_battery_cut_end:
            res = mcconf->l_battery_cut_end;
            break;

        case CFG_PARAM_l_temp_fet_start:
            res = mcconf->l_temp_fet_start;
            break;

        case CFG_PARAM_l_temp_fet_end:
            res = mcconf->l_temp_fet_end;
            break;

        case CFG_PARAM_l_temp_motor_start:
            res = mcconf->l_temp_motor_start;
            break;

        case CFG_PARAM_l_temp_motor_end:
            res = mcconf->l_temp_motor_end;
            break;

        case CFG_PARAM_l_temp_accel_dec:
            res = mcconf->l_temp_accel_dec;
            break;

        case CFG_PARAM_l_min_duty:
            res = mcconf->l_min_duty;
            break;

        case CFG_PARAM_l_max_duty:
            res = mcconf->l_max_duty;
            break;

        case CFG_PARAM_IMU_accel_confidence_decay:
            res = appconf->imu_conf.accel_confidence_decay;
            break;

        case CFG_PARAM_IMU_mahony_kp:
            res = appconf->imu_conf.mahony_kp;
            break;

        case CFG_PARAM_IMU_mahony_ki:
            res = appconf->imu_conf.mahony_ki;
            break;

        case CFG_PARAM_IMU_madgwick_beta:
            res = appconf->imu_conf.madgwick_beta;
            break;

        case CFG_PARAM_IMU_rot_roll:
            res = appconf->imu_conf.rot_roll;
            break;

        case CFG_PARAM_IMU_rot_pitch:
            res = appconf->imu_conf.rot_pitch;
            break;

        case CFG_PARAM_IMU_rot_yaw:
            res = appconf->imu_conf.rot_yaw;
            break;
...
static bool lib_set_cfg_float(CFG_PARAM p, float value)
{
    ...
    *appconf = *app_get_configuration();
    ...
    {
        case CFG_PARAM_l_current_max:
            mcconf->l_current_max = value;
            changed_mc = 1;
            res = true;
            break;

        case CFG_PARAM_l_current_min:
            mcconf->l_current_min = value;
            changed_mc = 1;
            res = true;
            break;

        case CFG_PARAM_l_in_current_max:
            mcconf->l_in_current_max = value;
            changed_mc = 1;
            res = true;
            break;

        case CFG_PARAM_l_in_current_min:
            mcconf->l_in_current_min = value;
            changed_mc = 1;
            res = true;
            break;

        case CFG_PARAM_l_abs_current_max:
            mcconf->l_abs_current_max = value;
            changed_mc = 1;
            res = true;
            break;

        case CFG_PARAM_l_min_erpm:
            mcconf->l_min_erpm = value;
            changed_mc = 1;
            res = true;
            break;

        case CFG_PARAM_l_max_erpm:
            mcconf->l_max_erpm = value;
            changed_mc = 1;
            res = true;
            break;

        case CFG_PARAM_l_erpm_start:
            mcconf->l_erpm_start = value;
            changed_mc = 1;
            res = true;
            break;

        case CFG_PARAM_l_max_erpm_fbrake:
            mcconf->l_max_erpm_fbrake = value;
            changed_mc = 1;
            res = true;
            break;

        case CFG_PARAM_l_max_erpm_fbrake_cc:
            mcconf->l_max_erpm_fbrake_cc = value;
            changed_mc = 1;
            res = true;
            break;

        case CFG_PARAM_l_min_vin:
            mcconf->l_min_vin = value;
            changed_mc = 1;
            res = true;
            break;

        case CFG_PARAM_l_max_vin:
            mcconf->l_max_vin = value;
            changed_mc = 1;
            res = true;
            break;

        case CFG_PARAM_l_battery_cut_start:
            mcconf->l_battery_cut_start = value;
            changed_mc = 1;
            res = true;
            break;

        case CFG_PARAM_l_battery_cut_end:
            mcconf->l_battery_cut_end = value;
            changed_mc = 1;
            res = true;
            break;

        case CFG_PARAM_l_temp_fet_start:
            mcconf->l_temp_fet_start = value;
            changed_mc = 1;
            res = true;
            break;

        case CFG_PARAM_l_temp_fet_end:
            mcconf->l_temp_fet_end = value;
            changed_mc = 1;
            res = true;
            break;

        case CFG_PARAM_l_temp_motor_start:
            mcconf->l_temp_motor_start = value;
            changed_mc = 1;
            res = true;
            break;

        case CFG_PARAM_l_temp_motor_end:
            mcconf->l_temp_motor_end = value;
            changed_mc = 1;
            res = true;
            break;

        case CFG_PARAM_l_temp_accel_dec:
            mcconf->l_temp_accel_dec = value;
            changed_mc = 1;
            res = true;
            break;

        case CFG_PARAM_l_min_duty:
            mcconf->l_min_duty = value;
            changed_mc = 1;
            res = true;
            break;

        case CFG_PARAM_l_max_duty:
            mcconf->l_max_duty = value;
            changed_mc = 1;
            res = true;
            break;

        case CFG_PARAM_IMU_accel_confidence_decay:
            appconf->imu_conf.accel_confidence_decay = value;
            changed_app = 1;
            res = true;
            break;

        case CFG_PARAM_IMU_mahony_kp:
            appconf->imu_conf.mahony_kp = value;
            changed_app = 1;
            res = true;
            break;

        case CFG_PARAM_IMU_mahony_ki:
            appconf->imu_conf.mahony_ki = value;
            changed_app = 1;
            res = true;
            break;

        case CFG_PARAM_IMU_madgwick_beta:
            appconf->imu_conf.madgwick_beta = value;
            changed_app = 1;
            res = true;
            break;

        case CFG_PARAM_IMU_rot_roll:
            appconf->imu_conf.rot_roll = value;
            changed_app = 1;
            res = true;
            break;

        case CFG_PARAM_IMU_rot_pitch:
            appconf->imu_conf.rot_pitch = value;
            changed_app = 1;
            res = true;
            break;

        case CFG_PARAM_IMU_rot_yaw:
            appconf->imu_conf.rot_yaw = value;
            changed_app = 1;
            res = true;
            break;
...
static bool lib_set_cfg_int(CFG_PARAM p, int value)
{
    ...
    *appconf = *app_get_configuration();
    ...
    {
        case CFG_PARAM_app_can_mode:
            appconf->can_mode = value;
            res = true;
            break;

        case CFG_PARAM_app_can_baud_rate:
            appconf->can_baud_rate = value;
            res = true;
            break;
...
lbm_value ext_load_native_lib(lbm_value* args, lbm_uint argn)
{
    ...
    if(!lib_init_done)
    {
        ...
        cif.cif.mc_get_tot_current_directional_filtered =
