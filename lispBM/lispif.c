...
void lispif_init(void)
{
    ...
    if(!timeout_had_IWDG_reset() &&
       (terminal_get_first_fault() != FAULT_CODE_BOOTING_FROM_WATCHDOG_RESET))
    {
...
void lispif_process_cmd(unsigned char* data, unsigned int len, void (* reply_func)(
                            unsigned char* data, unsigned int len))
{
    ...
    switch(packet_id)
    {
        ...
        case COMM_LISP_GET_STATS:
        {
            ...
            if(eval_tp)
            {
                cpu_use = 100.0 * (float) eval_tp->p_time /
                          (float) (chVTGetSystemTimeX() - time_last);
            ...
            {
                heap_use = 100.0 * (float) (HEAP_SIZE - lbm_heap_state.gc_last_free) /
                           (float) HEAP_SIZE;
            }

            mem_use = 100.0 * (float) (lbm_memory_num_words() - lbm_memory_num_free()) /
                      (float) lbm_memory_num_words();
            ...
            for(int i = 0; i < GLOBAL_ENV_ROOTS; i++)
            {
                ...
                while(lbm_type_of(curr) == LBM_TYPE_CONS)
                {
                    ...
                    if((lbm_type_of(lbm_car(key_val)) == LBM_TYPE_SYMBOL) &&
                       lbm_is_number(lbm_cdr(key_val)))
                    {
                            ...
                            ((name[1] == 't') || (name[1] == 'T'))))
                        {
                            ...
                            ind += strlen(name) + 1;
                            buffer_append_float32_auto(send_buffer_global,
                                                       lbm_dec_as_float(lbm_cdr(key_val)), &ind);
                        }
                    }
            ...
            if(lisp_thd_running)
            {
                ...
                else if(strncmp(str, ":info", 5) == 0)
                {
                    ...
                    commands_printf_lisp("GC SP max: %u (size %u)\n",
                                         lbm_heap_state.gc_stack.max_sp,
                                         lbm_heap_state.gc_stack.size);
                ...
                else if(strncmp(str, ":prof start", 11) == 0)
                {
                    ...
                    else
                    {
                        ...
                        {
                            commands_printf_lisp(
                                "Could not start profiler, most likely out of memory\n");
                        }
                    }
                }
                ...
                {
                    commands_printf_lisp(
                        "Profiler stopped. Issue command ':prof report' for statistics\n");
                    prof_running = false;
                }
                else if(strncmp(str, ":prof report", 12) == 0)
                {
                    ...
                    for(int i = 0; i < PROF_DATA_NUM; i++)
                    {
                        ...
                        commands_printf_lisp("%d\t%s\t%u\t%.3f\t%.3f",
                                             ...
                                             prof_data[i].name,
                                             prof_data[i].count,
                                             (double) (100.0 * ((float) prof_data[i].count) /
                                                       (float) tot_samples),
                                             (double) (100.0 * ((float) prof_data[i].gc_count) /
                                                       (float) prof_data[i].count));
                    }

                    commands_printf_lisp(" ");
                    commands_printf_lisp("GC:\t%u\t%f%%\n", tot_gc,
                                         (double) (100.0 * ((float) tot_gc / (float) tot_samples)));
                    commands_printf_lisp("System:\t%u\t%f%%\n", num_system,
                                         (double) (100.0 *
                                                   ((float) num_system / (float) tot_samples)));
                    commands_printf_lisp("Sleep:\t%u\t%f%%\n", num_sleep,
                                         (double) (100.0 *
                                                   ((float) num_sleep / (float) tot_samples)));
                ...
                {
                    commands_printf_lisp(lispif_restart(false,
                                                        flash_helper_code_size(CODE_IND_LISP) > 0,
                                                        true) ?
                                         "Reset OK\n\n" : "Reset Failed\n\n");
                }
                ...
                else if(strncmp(str, ":undef", 6) == 0)
                {
                    ...
                    commands_printf_lisp("%s", lbm_undefine(
                                             sym) ? "Cleared bindings" : "No definition found");
                    lbm_continue_eval();
                }
                ...
                else
                {
                    ...
                    if(ok)
                    {
                        ...
                        if(repl_buffer)
                        {
                            ...
                            lbm_create_string_char_channel(&string_tok_state, &string_tok,
                                                           repl_buffer);
...
bool lispif_restart(bool print, bool load_code, bool load_imports)
{
    ...
    if(!load_code || ((code_data != 0) && (code_len > 0)))
    {
        ...
        if(!lisp_thd_running)
        {
            ...
            chThdCreateStatic(eval_thread_wa, sizeof(eval_thread_wa), NORMALPRIO - 1, eval_thread,
                              NULL);
...
static bool const_heap_write(lbm_uint ix, lbm_uint w)
{
    ...
    {
        commands_printf_lisp("Attempted to write to const heap at %d, but it is already occupied",
                             ix);
        return false;
    }
