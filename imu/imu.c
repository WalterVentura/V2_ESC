...
void imu_init_icm20948(stm32_gpio_t* sda_gpio, int sda_pin, stm32_gpio_t* scl_gpio, int scl_pin,
                       int ad0_val)
{
    imu_stop();
...
void imu_init_bmi160_i2c(stm32_gpio_t* sda_gpio, int sda_pin, stm32_gpio_t* scl_gpio, int scl_pin)
{
    imu_stop();
...
void imu_init_bmi160_spi(stm32_gpio_t* nss_gpio, int nss_pin, stm32_gpio_t* sck_gpio, int sck_pin,
                         stm32_gpio_t* mosi_gpio, int mosi_pin, stm32_gpio_t* miso_gpio,
                         int miso_pin)
{
    imu_stop();
...
void imu_init_lsm6ds3(stm32_gpio_t* sda_gpio, int sda_pin, stm32_gpio_t* scl_gpio, int scl_pin)
{
    m_i2c_bb.sda_gpio = sda_gpio;
    m_i2c_bb.sda_pin = sda_pin;
    m_i2c_bb.scl_gpio = scl_gpio;
...
void imu_get_calibration(float yaw, float* imu_cal)
{
    ...
    float rotation1[3] =
    {
        DEG2RAD_f(m_settings.rot_roll), DEG2RAD_f(m_settings.rot_pitch), DEG2RAD_f(
            m_settings.rot_yaw)
    };
...
static void imu_read_callback(float* accel, float* gyro, float* mag)
{
    ...
    switch(m_settings.mode)
    {
        ...
        case AHRS_MODE_MADGWICK_FUSION:
        {
            ...
            FusionAhrsUpdateWithoutMagnetometer(&m_fusionAhrs, calibratedGyroscope,
                                                calibratedAccelerometer, dt);
