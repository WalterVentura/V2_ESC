...
#define Q fusionAhrs->quaternion.element // define shorthand label for more readable code
    ...
    do {
        ...
        if((accelerometer.axis.x == 0.0f) && (accelerometer.axis.y == 0.0f) &&
        ...
        halfFeedbackError = FusionVectorCrossProduct(FusionVectorFastNormalise(
                                                         accelerometer), halfGravity);
        ...
        halfFeedbackError =
            FusionVectorAdd(halfFeedbackError,
                            FusionVectorCrossProduct(FusionVectorFastNormalise(
    ...
    float accMag =
        sqrtf(SQ(accelerometer.axis.x) + SQ(accelerometer.axis.y) + SQ(accelerometer.axis.z));
    float accelConfidence = calculateAccConfidence(fusionAhrs->acc_conf_decay, accMag,
                                                   &fusionAhrs->accMagP);
    feedbackGain *= accelConfidence;

    // Convert gyroscope to radians per second scaled by 0.5
    FusionVector3 halfGyroscope = FusionVectorMultiplyScalar(gyroscope, 0.5f * FusionDegreesToRadians(
                                                                 1.0f));
    ...
    fusionAhrs->quaternion = FusionQuaternionAdd(fusionAhrs->quaternion, FusionQuaternionMultiplyVector(
...
#define A fusionAhrs->linearAcceleration.axis
    ...
    {
        .axis.x = 2.0f *
                  ((qwqw - 0.5f + Q.x * Q.x) * A.x + (qxqy - qwqz) * A.y + (qxqz + qwqy) * A.z),
        .axis.y = 2.0f *
                  ((qxqy + qwqz) * A.x + (qwqw - 0.5f + Q.y * Q.y) * A.y + (qyqz - qwqx) * A.z),
        .axis.z = 2.0f *
                  ((qxqz - qwqy) * A.x + (qyqz + qwqx) * A.y + (qwqw - 0.5f + Q.z * Q.z) * A.z),
