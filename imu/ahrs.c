...
void ahrs_update_all_parameters(ATTITUDE_INFO* att, float confidence_decay, float kp, float ki,
                                float beta)
{
...
void ahrs_update_mahony_imu(float* gyroXYZ, float* accelXYZ, float dt, ATTITUDE_INFO* att)
{
    ...
    if(accelNorm > 0.01)
    {
        ...
        accelConfidence =
            calculateAccConfidence(accelNorm, &att->accMagP, att->acc_confidence_decay);
    ...
    recipNorm = invSqrt(
        att->q0 * att->q0 + att->q1 * att->q1 + att->q2 * att->q2 + att->q3 * att->q3);
    att->q0 *= recipNorm;
    att->q1 *= recipNorm;
    att->q2 *= recipNorm;
    att->q3 *= recipNorm;
...
void ahrs_update_madgwick_imu(float* gyroXYZ, float* accelXYZ, float dt, ATTITUDE_INFO* att)
{
    ...
    if(accelNorm > 0.01)
    {
        ...
        s1 = _4q1 * q3q3 - _2q3 * ax + 4.0f * q0q0 * q1 - _2q0 * ay - _4q1 + _8q1 * q1q1 + _8q1 *
             q2q2 + _4q1 * az;
        s2 = 4.0f * q0q0 * q2 + _2q0 * ax + _4q2 * q3q3 - _2q3 * ay - _4q2 + _8q2 * q1q1 + _8q2 *
             q2q2 + _4q2 * az;
