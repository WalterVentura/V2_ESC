...
void lsm6ds3_init(i2c_bb_state* i2c_state, stkalign_t* work_area, size_t work_area_size)
{
    ...
    lsm6ds3_thread_ref = chThdCreateStatic(work_area, work_area_size, NORMALPRIO, lsm6ds3_thread,
                                           NULL);
}
...
static THD_FUNCTION(lsm6ds3_thread, arg)
{
    ...
    while(!chThdShouldTerminateX())
    {
        ...
        // Parse 6 axis values
        float gx =
            (float) ((int16_t) ((uint16_t) rxb[1] << 8) + rxb[0]) * 4.375 * (2000 / 125) / 1000;
        float gy =
            (float) ((int16_t) ((uint16_t) rxb[3] << 8) + rxb[2]) * 4.375 * (2000 / 125) / 1000;
        float gz =
            (float) ((int16_t) ((uint16_t) rxb[5] << 8) + rxb[4]) * 4.375 * (2000 / 125) / 1000;
        ...
        float az =
            (float) ((int16_t) ((uint16_t) rxb[11] << 8) + rxb[10]) * 0.061 * (16 >> 1) / 1000;
