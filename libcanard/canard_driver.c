...
static void updateParamByName(uint8_t* name, float value)
{
    ...
    if(p != NULL)
    {
        if(p->val != value)
        {
            if(debug_level > 0)
            {
                commands_printf("%s, %s p->val %0.02f, value %0.02f", p->name, name,
                                (double) p->val, (double) value);
            }
...
static param_t* getParamByIndex(uint16_t index)
{
    ...
    if(debug_level == 2)
    {
        commands_printf("Size of parameter array is: %d",
...
static void sendEscStatus(CanardInstance* ins)
{
    ...
    status.temperature =
        fmaxf(mc_interface_temp_fet_filtered(), mc_interface_temp_motor_filtered()) + 273.15;
...
static void handle_esc_raw_command(CanardInstance* ins, CanardRxTransfer* transfer)
{
    ...
    if(uavcan_equipment_esc_RawCommand_decode_internal(transfer, transfer->payload_len, &cmd, &tmp,
                                                       0) >= 0)
    {
        if(cmd.cmd.len > app_get_configuration()->uavcan_esc_index)
        {
            float raw_val = ((float) cmd.cmd.data[app_get_configuration()->uavcan_esc_index]) /
                            8192.0;
...
static void handle_esc_rpm_command(CanardInstance* ins, CanardRxTransfer* transfer)
{
    ...
    if(uavcan_equipment_esc_RPMCommand_decode_internal(transfer, transfer->payload_len, &cmd, &tmp,
                                                       0) >= 0)
    {
...
static void handle_esc_status(CanardInstance* ins, CanardRxTransfer* transfer)
{
    ...
    if(uavcan_equipment_esc_Status_decode_internal(transfer, transfer->payload_len, &msg, 0,
                                                   0) >= 0)
    {
        for(int i = 0; i < STATUS_MSGS_TO_STORE; i++)
        {
...
static void handle_param_getset(CanardInstance* ins, CanardRxTransfer* transfer)
{
    ...
    if(uavcan_protocol_param_GetSetRequest_decode(transfer, transfer->payload_len, &req,
                                                  &arraybuf_ptr) < 0)
    {
        return;
    }
...
static void handle_begin_firmware_update(CanardInstance* ins, CanardRxTransfer* transfer)
{
    ...
    uint32_t total_size =
        uavcan_protocol_file_BeginFirmwareUpdateResponse_encode(&reply, msg_buffer);
    canardRequestOrRespond(ins,
                           transfer->source_node_id,
...
static THD_FUNCTION(canard_thread, arg)
{
    ...
    for(;;)
    {
        ...
        if((conf->can_status_rate_2 > 0) &&
           (UTILS_AGE_S(last_esc_status_time_r2) >= (1.0 / (float) conf->can_status_rate_2)))
        {
            last_esc_status_time_r2 = chVTGetSystemTimeX();
        ...
        if(ST2MS(chVTTimeElapsedSinceX(last_param_refresh)) >= 1000 / PARAM_REFRESH_RATE_HZ)
        {
            ...
            if(debug_level == 7)
            {
                commands_printf("Refreshing Parameters Time Delta: %d",
                                ST2MS(chVTGetSystemTimeX() - last_param_refresh));
            }
        }
