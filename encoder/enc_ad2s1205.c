...
bool enc_ad2s1205_init(AD2S1205_config_t* cfg)
{
    ...
    palSetPadMode(AD2S1205_SAMPLE_GPIO, AD2S1205_SAMPLE_PIN,
    ...
    palSetPadMode(AD2S1205_RDVEL_GPIO, AD2S1205_RDVEL_PIN,
                  PAL_MODE_OUTPUT_PUSHPULL | PAL_STM32_OSPEED_HIGHEST);
...
void enc_ad2s1205_routine(AD2S1205_config_t* cfg)
{
    ...
    if(cfg->state.spi_val == 0) // an empty SPI packet means that the resolver IC is not responding
    {
        ...
        {
            cfg->state.resolver_VOIDspi_peak_error_rate =
                cfg->state.resolver_void_packet_error_rate;
        }
    }
    else
    {
        ...
        if(RDVEL == 0)
        {
            ...
            if(cfg->state.resolver_vel_packet_error_rate >
               cfg->state.resolver_VELread_peak_error_rate)
            {
                cfg->state.resolver_VELread_peak_error_rate =
                    cfg->state.resolver_vel_packet_error_rate;
            }
        }
        else
        {
    ...
    if((RDVEL != 0))
    {
        ...
        if(LOT)
        {
            ...
            if(cfg->state.resolver_loss_of_tracking_error_rate >
               cfg->state.resolver_LOT_peak_error_rate)
            {
                cfg->state.resolver_LOT_peak_error_rate =
                    cfg->state.resolver_loss_of_tracking_error_rate;
            }
        }
        else
        {
        ...
        if(DOS)
        {
            ...
            if(cfg->state.resolver_degradation_of_signal_error_rate >
               cfg->state.resolver_DOS_peak_error_rate)
            {
                cfg->state.resolver_DOS_peak_error_rate =
                    cfg->state.resolver_degradation_of_signal_error_rate;
            }
        }
        else
        {
        ...
        if(LOS)
        {
            ...
            if(cfg->state.resolver_loss_of_signal_error_rate >
               cfg->state.resolver_LOS_peak_error_rate)
            {
                cfg->state.resolver_LOS_peak_error_rate =
                    cfg->state.resolver_loss_of_signal_error_rate;
            }
        }
        else
        {
