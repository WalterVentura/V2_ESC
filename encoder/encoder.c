...
bool encoder_init(volatile mc_configuration* conf)
{
    ...
    switch(conf->m_sensor_port_mode)
    {
        ...
        case SENSOR_PORT_MODE_SINCOS:
        {
            ...
            sincosf(DEG2RAD_f(
                        conf->m_encoder_sincos_phase_correction), &encoder_cfg_sincos.sph,
                    &encoder_cfg_sincos.cph);
        ...
        switch(m_conf->m_sensor_port_mode)
        {
            case SENSOR_PORT_MODE_AS5047_SPI:
                ...
                if(encoder_cfg_as504x.sw_spi.mosi_gpio != NULL)
                {
                    ...
                    {
                        mc_interface_fault_stop(FAULT_CODE_ENCODER_NO_MAGNET, is_second_motor,
                                                false);
                    }
                    else if(diag.is_Comp_low)
                    {
                        mc_interface_fault_stop(FAULT_CODE_ENCODER_MAGNET_TOO_STRONG,
            ...
            case SENSOR_PORT_MODE_AS5x47U_SPI:
                ...
                {
                    mc_interface_fault_stop(FAULT_CODE_ENCODER_MAGNET_TOO_STRONG, is_second_motor,
                                            false);
                }
    ...
    switch(mcconf->m_sensor_port_mode)
    {
        ...
        case SENSOR_PORT_MODE_MT6816_SPI_HW:
            commands_printf("Low flux error (no magnet): errors: %d, error rate: %.3f %%",
                            ...
                            (double) (encoder_cfg_mt6816.state.encoder_no_magnet_error_rate *
                                      100.0));
            break;
        ...
        case SENSOR_PORT_MODE_TLE5012_SSC_SW:
            ...
            commands_printf(
                "Last error: %d, ssc error rate: %.3f %%, magnet strength: %d, temp %.2f C",
                status,
        ...
        case SENSOR_PORT_MODE_SINCOS:
            commands_printf(
                "Sin/Cos encoder signal below minimum amplitude: errors: %d, error rate: %.3f %%",
                encoder_cfg_sincos.state.signal_below_min_error_cnt,
                (double) (encoder_cfg_sincos.state.signal_low_error_rate * 100.0));

            commands_printf(
                "Sin/Cos encoder signal above maximum amplitude: errors: %d, error rate: %.3f %%",
                encoder_cfg_sincos.state.signal_above_max_error_cnt,
                (double) (encoder_cfg_sincos.state.signal_above_max_error_rate *
                          100.0));
            break;
        ...
        case SENSOR_PORT_MODE_AD2S1205:
            ...
            commands_printf("SPI encoder peak error rate: %.3f %%",
                            (double) encoder_cfg_ad2s1205.state.resolver_SPI_peak_error_rate *
                            (double) 100.0);
            commands_printf(
                "Resolver Loss Of Tracking (>5%c error): errors: %d, error rate: %.3f %%", 0xB0,
                encoder_cfg_ad2s1205.state.resolver_loss_of_tracking_error_cnt,
                (double) (encoder_cfg_ad2s1205.state.
                              resolver_loss_of_tracking_error_rate * 100.0));
            commands_printf("Resolver Loss Of Tracking peak error rate: %.3f %%",
                            (double) encoder_cfg_ad2s1205.state.resolver_LOT_peak_error_rate *
                            (double) 100.0);
            commands_printf(
                "Resolver Degradation Of Signal (>33%c error): errors: %d, error rate: %.3f %%",
                0xB0,
                encoder_cfg_ad2s1205.state.resolver_degradation_of_signal_error_cnt,
                (double) (encoder_cfg_ad2s1205.state.
                              resolver_degradation_of_signal_error_rate * 100.0));
            commands_printf("Resolver Degradation Of Signal peak error rate: %.3f %%",
                            (double) encoder_cfg_ad2s1205.state.resolver_DOS_peak_error_rate *
                            (double) 100.0);
            commands_printf(
            ...
            commands_printf("Resolver SPI empty packet: errors: %d, error rate: %.3f %%",
                            ...
                            (double) encoder_cfg_ad2s1205.state.resolver_void_packet_error_rate *
                            (double) 100.0);
            commands_printf("Resolver SPI empty packet peak error rate: %.3f %%",
                            (double) encoder_cfg_ad2s1205.state.resolver_VOIDspi_peak_error_rate *
                            (double) 100.0);
            commands_printf("Resolver angle velocity readings: errors: %d, error rate: %.3f %%",
                            ...
                            (double) encoder_cfg_ad2s1205.state.resolver_vel_packet_error_rate *
                            (double) 100.0);
            commands_printf("Resolver angle velocity readings peak error rate: %.3f %%",
                            (double) encoder_cfg_ad2s1205.state.resolver_VELread_peak_error_rate *
                            (double) 100.0);
            commands_printf("\n");
        ...
        case SENSOR_PORT_MODE_AS5x47U_SPI:
            commands_printf("SPI AS5x47U encoder value: %d, errors: %d, error rate: %.3f %%",
                            encoder_cfg_as5x47u.state.spi_val,
                            encoder_cfg_as5x47u.state.spi_communication_error_count,
        ...
        case SENSOR_PORT_MODE_BISSC:
            commands_printf(
                "BissC Loss SPI communication (>4%c error): errors: %d, error rate: %.3f %%", 0xB0,
                encoder_cfg_bissc.state.spi_comm_error_cnt,
                (double) (encoder_cfg_bissc.state.spi_comm_error_rate * 100.0));
            commands_printf(
                "BissC Degradation Of Signal (>5%c error): errors: %d, error rate: %.3f %%", 0xB0,
...
static THD_FUNCTION(routine_thread, arg)
{
    ...
    for(;;)
    {
        ...
        {
            case routine_rate_1k:
                chThdSleep(CH_CFG_ST_FREQUENCY / 1000);
                break;

            case routine_rate_2k:
                chThdSleep(CH_CFG_ST_FREQUENCY / 2000);
                break;

            case routine_rate_5k:
                chThdSleep(CH_CFG_ST_FREQUENCY / 5000);
                break;

            case routine_rate_10k:
                chThdSleep(CH_CFG_ST_FREQUENCY / 10000);
                break;
...
static void timer_start(routine_rate_t rate)
{
    ...
    if(!routine_running)
    {
        ...
        chThdCreateStatic(routine_thread_wa, sizeof(routine_thread_wa), NORMALPRIO + 5,
                          routine_thread, NULL);
    }
}
