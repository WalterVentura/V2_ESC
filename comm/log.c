...
void log_start(int can_id, int field_num, float rate_hz, bool append_time, bool append_gnss,
...
void log_config_field(int can_id, int field_ind, char* key, char* name, char* unit, int precision,
                      bool is_relative, bool is_timestamp)
{
    ...
    strcpy((char*) (buffer + ind), key);
    ind += key_len + 1;
    strcpy((char*) (buffer + ind), name);
    ind += name_len + 1;
    strcpy((char*) (buffer + ind), unit);
    ind += unit_len + 1;
...
void log_send_samples_f32(int can_id, int field_start, float* samples, int sample_num)
{
    if(sample_num > 100)
    {
        return;
...
void log_send_samples_f64(int can_id, int field_start, double* samples, int sample_num)
{
    if(sample_num > 50)
    {
        return;
