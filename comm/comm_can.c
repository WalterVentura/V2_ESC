...
void comm_can_send_buffer(uint8_t controller_id, uint8_t* data, unsigned int len, uint8_t send)
{
    ...
    else
    {
        ...
        for(unsigned int i = 0; i < len; i += 7)
        {
            ...
            comm_can_transmit_eid_replace(controller_id |
                                          ((uint32_t) CAN_PACKET_FILL_RX_BUFFER << 8), send_buffer,
                                          send_len + 1, true, 0);
        }
...
void comm_can_set_current_brake(uint8_t controller_id, float current)
{
    ...
    buffer_append_int32(buffer, (int32_t) (current * 1000.0), &send_index);
    comm_can_transmit_eid_replace(controller_id |
                                  ((uint32_t) CAN_PACKET_SET_CURRENT_BRAKE << 8), buffer,
                                  send_index, true, 0);
}
...
void comm_can_set_current_brake_rel(uint8_t controller_id, float current_rel)
{
    ...
    buffer_append_float32(buffer, current_rel, 1e5, &send_index);
    comm_can_transmit_eid_replace(controller_id |
                                  ((uint32_t) CAN_PACKET_SET_CURRENT_BRAKE_REL << 8), buffer,
                                  send_index, true, 0);
}
...
void comm_can_set_handbrake(uint8_t controller_id, float current)
{
    ...
    comm_can_transmit_eid_replace(controller_id |
                                  ((uint32_t) CAN_PACKET_SET_CURRENT_HANDBRAKE << 8), buffer,
                                  send_index, true, 0);
}
...
void comm_can_set_handbrake_rel(uint8_t controller_id, float current_rel)
{
    ...
    buffer_append_float32(buffer, current_rel, 1e5, &send_index);
    comm_can_transmit_eid_replace(controller_id |
                                  ((uint32_t) CAN_PACKET_SET_CURRENT_HANDBRAKE_REL << 8), buffer,
                                  send_index, true, 0);
}
 ...
 */
void comm_can_detect_apply_all_foc(uint8_t controller_id, bool activate_status_msgs,
                                   float max_power_loss)
{
    ...
    comm_can_transmit_eid_replace(controller_id |
                                  ((uint32_t) CAN_PACKET_DETECT_APPLY_ALL_FOC << 8), buffer,
                                  send_index, true, 0);
}
...
void comm_can_conf_current_limits(uint8_t controller_id, bool store, float min, float max)
{
    ...
    comm_can_transmit_eid_replace(controller_id |
                                  ((uint32_t) (store ? CAN_PACKET_CONF_STORE_CURRENT_LIMITS :
                                               CAN_PACKET_CONF_CURRENT_LIMITS) << 8), buffer,
                                  send_index, true, 0);
}
...
void comm_can_conf_current_limits_in(uint8_t controller_id, bool store, float min, float max)
{
    ...
    comm_can_transmit_eid_replace(controller_id |
                                  ((uint32_t) (store ? CAN_PACKET_CONF_STORE_CURRENT_LIMITS_IN :
                                               CAN_PACKET_CONF_CURRENT_LIMITS_IN) << 8), buffer,
                                  send_index, true, 0);
}
 ...
 */
void comm_can_conf_foc_erpms(uint8_t controller_id, bool store, float foc_openloop_rpm,
                             float foc_sl_erpm)
{
    int32_t send_index = 0;
    uint8_t buffer[8];
...
void comm_can_conf_battery_cut(uint8_t controller_id, bool store, float start, float end)
{
    int32_t send_index = 0;
    uint8_t buffer[8];
    ...
    comm_can_transmit_eid_replace(controller_id |
                                  ((uint32_t) (store ? CAN_PACKET_CONF_STORE_BATTERY_CUT :
                                               CAN_PACKET_CONF_BATTERY_CUT) << 8), buffer,
                                  send_index, true, 0);
}
...
void comm_can_send_status1(uint8_t id, bool replace)
{
    ...
    buffer_append_int16(buffer,
                        (int16_t) (mc_interface_get_tot_current_filtered() * 1e1), &send_index);
...
void comm_can_send_status2(uint8_t id, bool replace)
{
    ...
    buffer_append_int32(buffer, (int32_t) (mc_interface_get_amp_hours_charged(
                                               false) * 1e4), &send_index);
...
void comm_can_send_status3(uint8_t id, bool replace)
{
    ...
    buffer_append_int32(buffer, (int32_t) (mc_interface_get_watt_hours_charged(
                                               false) * 1e4), &send_index);
...
void comm_can_send_status4(uint8_t id, bool replace)
{
    ...
    buffer_append_int16(buffer,
                        (int16_t) (mc_interface_get_tot_current_in_filtered() * 1e1), &send_index);
...
void comm_can_send_status5(uint8_t id, bool replace)
{
    ...
    buffer_append_int16(buffer,
                        (int16_t) (mc_interface_get_input_voltage_filtered() * 1e1), &send_index);
        ...
        switch(cmd)
        {
            ...
            case CAN_PACKET_PROCESS_RX_BUFFER:
            {
                    ...
                    (unsigned short) crc_low))
                {
                    ...
                    switch(commands_send)
                    {
                        case 0:
                        case 3:
                            commands_process_packet(rx_buffer[buf_ind], rxbuf_len,
                                                    send_packet_wrapper);
                            break;
            ...
            case CAN_PACKET_POLL_TS5700N8501_STATUS:
            {
                comm_can_transmit_eid_replace(app_get_configuration()->controller_id |
                                              ...
                                              enc_ts5700n8501_get_raw_status(&
                                                                             encoder_cfg_TS5700N8501), 8, true,
                                              0);
            } break;
