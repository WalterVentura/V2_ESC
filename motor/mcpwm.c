    ...
    else
    {
        if(sensorless_now)
        {
            if(state != MC_STATE_RUNNING)
            {
                ...
                {
                    if((fabsf(rpm_now) < conf->sl_min_erpm) &&
                       (mcpwm_get_tot_current_filtered() <
                        conf->sl_max_fullbreak_current_dir_change))
                    {
                        state = MC_STATE_RUNNING;
                        commutate(1);
                    }
...
static void set_duty_cycle_hw(float dutyCycle)
{
    ...
    if((conf->motor_type == MOTOR_TYPE_BLDC) && (conf->pwm_mode == PWM_MODE_BIPOLAR) &&
       !IS_DETECTING())
    {
...
static THD_FUNCTION(rpm_thread, arg)
{
    ...
    for(;;)
    {
        ...
        rpm_dep.cycle_int_limit_running = rpm_dep.cycle_int_limit +
                                          (float) ADC_Value[ADC_IND_VIN_SENS] *
                                          conf->sl_bemf_coupling_k /
                                          (rpm_abs >
                                           conf->sl_min_erpm ? rpm_abs : conf->sl_min_erpm);
        rpm_dep.cycle_int_limit_running = utils_map(rpm_abs, 0,
                                                    conf->sl_cycle_int_rpm_br,
                                                    rpm_dep.cycle_int_limit_running,
                                                    rpm_dep.cycle_int_limit_running *
                                                    conf->sl_phase_advance_at_br);
    ...
    for(;;)
    {
        ...
        if(state == MC_STATE_OFF)
        {
            ...
            amp = filter_run_fir_iteration((float*) amp_fir_samples,
                                           (float*) amp_fir_coeffs, AMP_FIR_TAPS_BITS,
                                           amp_fir_index);
        ...
        if(sensorless_now)
        {
            ...
            if(((state == MC_STATE_RUNNING) && (pwm_cycles >= 2)) || (state == MC_STATE_OFF))
            {
                ...
                if(v_diff > 0)
                {
                    ...
                    // const int min = 100;
                    int min =
                        (int) ((1.0 - fabsf(dutycycle_now)) * (float) ADC_Value[ADC_IND_VIN_SENS] *
                               0.3);
                    ...
                    if((pwm_cycles_sum > (last_pwm_cycles_sum / 2.0)) ||
                       !has_commutated ||
                       ((ph_now_raw > min) && (ph_now_raw < (ADC_Value[ADC_IND_VIN_SENS] - min))))
                    {
    ...
    if((state == MC_STATE_RUNNING) && has_commutated)
    {
        ...
        if(current_nofilter > conf->lo_current_max)
        {
            utils_step_towards((float*) &dutycycle_now, 0.0,
                               ramp_step_no_lim * fabsf(
                                   current_nofilter - conf->lo_current_max) *
                               conf->m_current_backoff_gain);
            limit_delay = 1;
        }
        else if(current_nofilter < conf->lo_current_min)
        {
            utils_step_towards((float*) &dutycycle_now,
                               direction ? conf->l_max_duty : -conf->l_max_duty,
                               ramp_step_no_lim * fabsf(
                                   current_nofilter - conf->lo_current_min) *
                               conf->m_current_backoff_gain);
            limit_delay = 1;
        }
        else if(current_in_nofilter > conf->lo_in_current_max)
        {
            utils_step_towards((float*) &dutycycle_now, 0.0,
                               ramp_step_no_lim *
                               fabsf(
                                   current_in_nofilter - conf->lo_in_current_max) *
                               conf->m_current_backoff_gain);
            limit_delay = 1;
        }
        else if(current_in_nofilter < conf->lo_in_current_min)
        {
            utils_step_towards((float*) &dutycycle_now,
                               ...
                               ramp_step_no_lim *
                               fabsf(
                                   current_in_nofilter - conf->lo_in_current_min) *
                               conf->m_current_backoff_gain);
            limit_delay = 1;
        }
