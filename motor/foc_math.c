...
void foc_observer_update(float v_alpha, float v_beta, float i_alpha, float i_beta, float dt,
                         observer_state* state, float* phase, motor_all_state_t* motor)
{
...
void foc_pll_run(float phase, float dt, float* phase_var, float* speed_var, mc_configuration* conf)
{
 ...
 */
void foc_svm(float alpha, float beta, uint32_t PWMFullDutyCycle, uint32_t* tAout, uint32_t* tBout,
...
void foc_run_pid_control_pos(bool index_found, float dt, motor_all_state_t* motor)
{
    ...
    {
        d_term_proc =
            -utils_angle_difference(angle_now,
                                    motor->m_pos_prev_proc) * error_sign *
            (kd_proc / motor->m_pos_dt_int_proc);
...
void foc_run_pid_control_speed(bool index_found, float dt, motor_all_state_t* motor)
{
    ...
    if(conf_now->s_pid_ramp_erpms_s > 0.0)
    {
        ...
        utils_truncate_number(&motor->m_speed_pid_set_rpm, conf_now->l_min_erpm,
                              conf_now->l_max_erpm);
    }
...
float foc_correct_hall(float angle, float dt, motor_all_state_t* motor, int hall_val)
{
    ...
    if(ang_hall_int < 201)
    {
        ...
        if(RADPS2RPM_f((M_PI / 3.0) /
                       fmaxf(motor->m_hall_dt_diff_now, motor->m_hall_dt_diff_last)) <
        ...
        // changes fast.
        float angle_step =
            (fmaxf(rpm_abs_hall, conf_now->foc_hall_interp_erpm) / 60.0) * 2.0 * M_PI * dt * 1.4;
        float angle_diff = utils_angle_difference_rad(motor->m_ang_hall,
                                                      motor->m_ang_hall_rate_limited);
    ...
    if(angle_old != angle)
    {
        float weight_hall = utils_map(rpm_abs, conf_now->foc_sl_erpm_start, conf_now->foc_sl_erpm,
                                      1.0, 0.0);
        ...
        (motor->m_i_fw_set > motor->m_conf->cc_min_current)))
    {
        ...
        else
        {
            utils_step_towards((float*) &motor->m_i_fw_set, fw_current_now,
                               ...
                               motor->m_conf->foc_fw_current_max);
        }
    }
}
