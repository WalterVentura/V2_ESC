...
static void timer_reinit(int f_zv)
{
    ...
    TIM_BDTRInitStructure.TIM_DeadTime = conf_general_calculate_deadtime(HW_DEAD_TIME_NSEC,
                                                                         SYSTEM_CORE_CLOCK);
...
void mcpwm_foc_init(mc_configuration* conf_m1, mc_configuration* conf_m2)
{
    ...
    RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_DMA2 | RCC_AHB1Periph_GPIOA | RCC_AHB1Periph_GPIOC,
                           ENABLE);
    ...
    if(m_motor_1.m_conf->foc_offsets_cal_on_boot)
    {
        ...
        if(!m_dccal_done)
        {
            ...
            m_motor_1.m_conf->foc_offsets_voltage_undriven[0] =
                MCCONF_FOC_OFFSETS_VOLTAGE_UNDRIVEN_0;
            m_motor_1.m_conf->foc_offsets_voltage_undriven[1] =
                MCCONF_FOC_OFFSETS_VOLTAGE_UNDRIVEN_1;
            m_motor_1.m_conf->foc_offsets_voltage_undriven[2] =
                MCCONF_FOC_OFFSETS_VOLTAGE_UNDRIVEN_2;
            ...
            m_motor_2.m_conf->foc_offsets_voltage_undriven[0] =
                MCCONF_FOC_OFFSETS_VOLTAGE_UNDRIVEN_0;
            m_motor_2.m_conf->foc_offsets_voltage_undriven[1] =
                MCCONF_FOC_OFFSETS_VOLTAGE_UNDRIVEN_1;
            m_motor_2.m_conf->foc_offsets_voltage_undriven[2] =
                MCCONF_FOC_OFFSETS_VOLTAGE_UNDRIVEN_2;
...
void mcpwm_foc_set_openloop_current(float current, float rpm)
{
    utils_truncate_number(&current,
                          ...
                          get_motor_now()->m_conf->l_current_max *
                          get_motor_now()->m_conf->l_current_max_scale);
...
void mcpwm_foc_get_voltage_offsets(float* v0_offset, float* v1_offset, float* v2_offset,
...
void mcpwm_foc_get_voltage_offsets_undriven(float* v0_offset, float* v1_offset, float* v2_offset,
                                            bool is_second_motor)
{
    volatile motor_all_state_t* motor = M_MOTOR(is_second_motor);
...
float mcpwm_foc_get_tot_current_filtered_motor(bool is_second_motor)
{
    ...
    return SIGN(motor->m_motor_state.vq * motor->m_motor_state.iq_filter) *
           motor->m_motor_state.i_abs_filter;
}
...
float mcpwm_foc_get_est_ind(void)
{
    ...
    get_motor_now()->m_hfi.fft_bin0_func(
        (float*) get_motor_now()->m_hfi.buffer, &real_bin0, &imag_bin0);
    return real_bin0;
}
...
int mcpwm_foc_encoder_detect(float current, bool print, float* offset, float* ratio, bool* inverted)
{
    ...
    for(int i = 0; i < it_rat; i++)
    {
        ...
        if((i > 3) &&
           (fabsf(utils_angle_difference_rad(motor->m_phase_now_encoder,
                                             first)) < fabsf(diff / 2.0)))
        {
            break;
        }
    }
    ...
    for(int i = 0; i < it_rat; i++)
    {
        ...
        for(float j = phase_ovr_tmp;
            j > phase_ovr_tmp - (2.0 / 3.0) * M_PI;
            j -= (2.0 * M_PI) / 500.0)
        {
            motor->m_phase_now_override = j;
            fault = mc_interface_get_fault();
    ...
    *ratio = roundf(((2.0 / 3.0) * 180.0) / fabsf(diff));
    ...
    for(int i = 0; i < it_ofs; i++)
    {
        ...
        float angle_diff = utils_angle_difference_rad(motor->m_phase_now_encoder,
                                                      motor->m_phase_now_override);
        float s, c;
        ...
        if(print)
        {
            commands_printf("Ovr: %.2f/%.2f Diff: %.2f", (double) override,
                            (double) (it_ofs * step), (double) RAD2DEG_f(angle_diff));
        }
    }
...
int mcpwm_foc_measure_resistance(float current, int samples, bool stop_after, float* resistance)
{
    ...
    const float current_avg = motor->m_samples.avg_current_tot /
                              (float) motor->m_samples.sample_num;
    const float voltage_avg = motor->m_samples.avg_voltage_tot /
                              (float) motor->m_samples.sample_num;
 ...
 * The fault code
 */
int mcpwm_foc_measure_inductance(float duty, int samples, float* curr, float* ld_lq_diff,
                                 float* inductance)
{
    volatile motor_all_state_t* motor = get_motor_now();
    int fault = FAULT_CODE_NONE;
    ...
    for(int i = 0; i < (samples / 10); i++)
    {
        ...
        motor->m_hfi.fft_bin0_func((float*) motor->m_hfi.buffer_current, &real_bin0_i,
                                   &imag_bin0_i);
 ...
 * The fault code
 */
int mcpwm_foc_measure_inductance_current(float curr_goal, int samples, float* curr,
...
int mcpwm_foc_dc_cal(bool cal_undriven)
{
    ...
    if(cal_undriven)
    {
        ...
        voltage_sum[2] = 0.0;
#ifdef HW_HAS_DUAL_MOTORS
        voltage_sum_m2[0] = 0.0;
        voltage_sum_m2[1] = 0.0;
        voltage_sum_m2[2] = 0.0;
#endif
...
void mcpwm_foc_adc_int_handler(void* p, uint32_t flags)
{
    ...
    motor_all_state_t* motor_now =
        is_second_motor ? (motor_all_state_t*) &m_motor_2 : (motor_all_state_t*) &m_motor_1;
    motor_all_state_t* motor_other =
        is_second_motor ? (motor_all_state_t*) &m_motor_1 : (motor_all_state_t*) &m_motor_2;
    ...
    if(motor_other->m_duty_next_set)
    {
        ...
        if(is_second_motor)
        {
            ...
            TIMER_UPDATE_DUTY_M1(motor_other->m_duty1_next, motor_other->m_duty2_next,
                                 motor_other->m_duty3_next);
        }
        else
        {
            ...
            TIMER_UPDATE_DUTY_M2(motor_other->m_duty1_next, motor_other->m_duty2_next,
                                 motor_other->m_duty3_next);
        }
    ...
    if((conf_other->foc_control_sample_mode == FOC_CONTROL_SAMPLE_MODE_V0_V7_INTERPOL) &&
       !skip_interpolation)
    {
        float interpolated_phase = motor_other->m_motor_state.phase +
                                   motor_other->m_speed_est_fast * dt * 0.5;
...
// Scale to AMPs using calibrated scaling factors
    ...
    if(motor_now->m_state == MC_STATE_RUNNING)
    {
        if(conf_now->foc_current_sample_mode == FOC_CURRENT_SAMPLE_MODE_ALL_SENSORS)
        {
            ...
            motor_now->m_motor_state.i_alpha = (2.0 / 3.0) * ia - (1.0 / 3.0) * ib - (1.0 / 3.0) *
                                               ic;
        ...
        {
            motor_now->m_motor_state.i_alpha = 0.5 *
                                               (motor_now->m_motor_state.i_alpha +
                                                motor_now->m_i_alpha_sample_next);
            motor_now->m_motor_state.i_beta = 0.5 *
                                              (motor_now->m_motor_state.i_beta +
                                               motor_now->m_i_beta_sample_next);
        ...
        if(motor_now->m_control_mode == CONTROL_MODE_CURRENT_BRAKE)
        {
            if(((SIGN(speed_fast_now) != SIGN(motor_now->m_br_speed_before)) ||
                ...
                (fabsf(motor_now->m_duty_filtered) < 0.001) ||
                (motor_now->m_br_no_duty_samples < 10)) &&
        ...
        if(!control_duty && motor_now->m_was_control_duty)
        {
            ...
            {
                motor_now->m_motor_state.vq_int -= motor_now->m_pll_speed *
                                                   conf_now->foc_motor_flux_linkage;
            }
        }
        ...
        // Set motor phase
        {
            ...
            {
                foc_observer_update(motor_now->m_motor_state.v_alpha,
                                    motor_now->m_motor_state.v_beta,
                                    motor_now->m_motor_state.i_alpha,
                                    motor_now->m_motor_state.i_beta,
                                    dt, &(motor_now->m_observer_state),
                                    &motor_now->m_phase_now_observer, motor_now);
                ...
                motor_now->m_phase_now_observer += motor_now->m_pll_speed * dt *
                                                   (0.5 + conf_now->foc_observer_offset);
            ...
            switch(conf_now->foc_sensor_mode)
            {
                ...
                case FOC_SENSOR_MODE_HALL:
                    motor_now->m_motor_state.phase = foc_correct_hall(
                        motor_now->m_phase_now_observer, dt, motor_now,
                        utils_read_hall(motor_now !=
                                        &m_motor_1, conf_now->m_hall_extra_samples));
        ...
        if((conf_now->foc_mtpa_mode != MTPA_MODE_OFF) && (ld_lq_diff != 0.0))
        {
            ...
            id_set_tmp = (lambda - sqrtf(SQ(lambda) + 8.0 * SQ(ld_lq_diff * iq_ref))) /
                         (4.0 * ld_lq_diff);
...
// run_fw(motor_now, dt);
        ...
        if(mod_q > 0.001)
        {
            utils_truncate_number(&iq_set_tmp, conf_now->lo_in_current_min / mod_q,
                                  conf_now->lo_in_current_max / mod_q);
        }
        else if(mod_q < -0.001)
        {
            utils_truncate_number(&iq_set_tmp, conf_now->lo_in_current_max / mod_q,
        ...
        {
            utils_truncate_number(&iq_set_tmp, -conf_now->lo_current_max,
                                  -conf_now->lo_current_min);
        }

        float current_max_abs =
            fabsf(utils_max_abs(conf_now->lo_current_max, conf_now->lo_current_min));
        ...
        // Set motor phase
        {
            switch(conf_now->foc_sensor_mode)
            {
                ...
                case FOC_SENSOR_MODE_HFI_START:
                {
                    ...
                    if(fabsf(RADPS2RPM_f(motor_now->m_pll_speed)) <
                       (conf_now->foc_sl_erpm_hfi * 1.1))
                    {
                        motor_now->m_hfi.est_done_cnt = 0;
                        motor_now->m_hfi.flip_cnt = 0;
                    }
    ...
    motor_now->m_motor_state.duty_now = SIGN(motor_now->m_motor_state.vq) *
                                        NORM2_f(motor_now->m_motor_state.mod_d,
                                                motor_now->m_motor_state.mod_q) * TWO_BY_SQRT3;
    ...
    foc_pll_run(phase_for_speed_est, dt, &motor_now->m_pll_phase, &motor_now->m_pll_speed,
                conf_now);

    // Low latency speed estimation, for e.g. HFI and speed control.
    {
        float diff = utils_angle_difference_rad(phase_for_speed_est,
                                                motor_now->m_phase_before_speed_est);
        ...
        float diff_corr = utils_angle_difference_rad(motor_now->m_motor_state.phase,
                                                     motor_now->m_phase_before_speed_est_corrected);
        ...
        utils_truncate_number_abs((float*) &motor_now->m_pll_speed,
                                  fabsf(motor_now->m_speed_est_fast) * 3.0);
...
static void timer_update(motor_all_state_t* motor, float dt)
{
    ...
    {
        float comp_fact = 1.0 + 0.00386 *
                          (mc_interface_temp_motor_filtered() - conf_now->foc_temp_comp_base_temp);
    ...
    if(motor->m_min_rpm_timer > 0.0)
    {
        ...
        float s, c;
        utils_fast_sincos_better(motor->m_phase_now_observer_override +
                                 SIGN(motor->m_motor_state.duty_now) * M_PI / 4.0, &s, &c);
    ...
    // https://doi.org/10.1002/acs.2587
    {
        ...
        motor->m_res_est = motor->m_r_est_state - 0.5 * res_est_gain * conf_now->foc_motor_l *
                           i_abs_sq;
        float res_dot = -res_est_gain * (motor->m_res_est * i_abs_sq + motor->m_speed_est_fast *
                                         (motor->m_motor_state.i_beta * motor->m_observer_state.x1 -
                                          motor->m_motor_state.i_alpha *
                                          motor->m_observer_state.x2) -
                                         (motor->m_motor_state.i_alpha *
                                          motor->m_motor_state.v_alpha +
                                          motor->m_motor_state.i_beta *
                                          motor->m_motor_state.v_beta));
        motor->m_r_est_state += res_dot * dt;

        utils_truncate_number((float*) &motor->m_r_est_state, conf_now->foc_motor_r * 0.25,
                              conf_now->foc_motor_r * 3.0);
    }
}
...
static void hfi_update(volatile motor_all_state_t* motor, float dt)
{
    ...
    if(motor->m_hfi.ready)
    {
                ...
                (motor->m_hfi.est_done_cnt >= motor->m_conf->foc_hfi_start_samples))
        {
            ...
            commands_plot_set_graph(1);
            commands_send_plot_points(motor->m_hfi_plot_sample, RAD2DEG_f(
                                          motor->m_phase_now_encoder) / 4e6);
        ...
        else
        {
            ...
            if(motor->m_hfi_plot_en == 1)
            {
                ...
                if(hfi_plot_div >= 8)
                {
                    ...
                    float real_bin0, imag_bin0;
                    motor->m_hfi.fft_bin0_func((float*) motor->m_hfi.buffer, &real_bin0,
                                               &imag_bin0);
...
// commands_send_plot_points(motor->m_hfi_plot_sample, motor->m_speed_est_fast);
            ...
            else if(motor->m_hfi_plot_en == 2)
            {
                ...
                if(hfi_plot_div >= 8)
                {
                    ...
                    commands_plot_set_graph(0);
                    commands_send_plot_points(motor->m_hfi_plot_sample,
                                              motor->m_hfi.buffer_current[(int) motor->
                                                                              m_hfi_plot_sample]);

                    commands_plot_set_graph(1);
                    commands_send_plot_points(motor->m_hfi_plot_sample,
                                              motor->m_hfi.buffer[(int) motor->m_hfi_plot_sample] *
                                              1e6);
...
static void control_current(motor_all_state_t* motor, float dt)
{
    ...
    if((motor->m_control_mode < CONTROL_MODE_HANDBRAKE) &&
       (conf_now->foc_cc_decoupling != FOC_CC_DECOUPLING_DISABLED))
    {
...
#else
    state_m->i_bus = state_m->mod_alpha_measured * state_m->i_alpha + state_m->mod_beta_measured *
                     state_m->i_beta;
    ...
    switch(audio->mode)
    {
        case MC_AUDIO_TABLE:
        {
            ...
            utils_saturate_vector_2d((float*) &state_m->mod_alpha_raw,
                                     (float*) &state_m->mod_beta_raw, SQRT3_BY_2 * 0.95);
        } break;
        ...
        case MC_AUDIO_SAMPLED:
        {
            ...
            float sample = (float) table[(int) floorf(audio->sample_pos)] / 128.0 *
                           audio->sample_voltage;
    ...
    if(do_hfi)
    {
        ...
        utils_truncate_number_abs(&hfi_voltage, state_m->v_bus * (1.0 - fabsf(
                                                                      state_m->duty_now)) * SQRT3_BY_2 *
                                  (2.0 / 3.0) * 0.95);

        if(((conf_now->foc_sensor_mode == FOC_SENSOR_MODE_HFI_V4) ||
            (conf_now->foc_sensor_mode == FOC_SENSOR_MODE_HFI_V5)) && hfi_est_done)
        {
            if(motor->m_hfi.is_samp_n)
            {
                float sample_now = c * motor->m_i_beta_sample_with_offset - s *
                                   motor->m_i_alpha_sample_with_offset;
                ...
                {
                    motor->m_hfi.prev_sample = c * motor->m_i_beta_sample_next - s *
                                               motor->m_i_alpha_sample_next;
        ...
        else if(((conf_now->foc_sensor_mode == FOC_SENSOR_MODE_HFI_V2) ||
                 (conf_now->foc_sensor_mode == FOC_SENSOR_MODE_HFI_V3)) && hfi_est_done)
        {
            if(motor->m_hfi.is_samp_n)
            {
                ...
                {
                    motor->m_hfi.prev_sample = motor->m_hfi.cos_last *
                                               motor->m_i_alpha_sample_next +
            ...
            {
                motor->m_hfi.prev_sample = motor->m_hfi.cos_last * state_m->i_alpha +
                                           motor->m_hfi.sin_last * state_m->i_beta;
                mod_alpha_v7 -= hfi_voltage * motor->m_hfi.cos_last * voltage_normalize;
                mod_beta_v7 -= hfi_voltage * motor->m_hfi.sin_last * voltage_normalize;
            }
        }
        else
        {
            ...
            {
                float sample_now =
                    (utils_tab_sin_32_1[motor->m_hfi.ind * motor->m_hfi.table_fact] *
                     state_m->i_alpha -
                     utils_tab_cos_32_1[motor->m_hfi.ind * motor->m_hfi.table_fact] *
                     state_m->i_beta);
                float di = (sample_now - motor->m_hfi.prev_sample);
                ...
                mod_alpha_v7 += hfi_voltage *
                                utils_tab_sin_32_1[motor->m_hfi.ind * motor->m_hfi.table_fact] *
                                voltage_normalize;
                mod_beta_v7 -= hfi_voltage *
                               utils_tab_cos_32_1[motor->m_hfi.ind * motor->m_hfi.table_fact] *
                               voltage_normalize;
            }
            else
            {
                motor->m_hfi.prev_sample =
                    utils_tab_sin_32_1[motor->m_hfi.ind * motor->m_hfi.table_fact] *
                    state_m->i_alpha -
                    utils_tab_cos_32_1[motor->m_hfi.ind *
                                       motor->m_hfi.table_fact] * state_m->i_beta;
    ...
    foc_svm(state_m->mod_alpha_raw, state_m->mod_beta_raw, top, &duty1, &duty2, &duty3,
            (uint32_t*) &state_m->svm_sector);
...
static void update_valpha_vbeta(motor_all_state_t* motor, float mod_alpha, float mod_beta)
{
    ...
    const float mod_alpha_filter_sgn = (1.0 / 3.0) *
                                       (2.0 * SIGN(ia_filter) - SIGN(ib_filter) - SIGN(ic_filter));
    ...
    if(motor->m_state == MC_STATE_RUNNING)
    {
        ...
        if(conf_now->foc_phase_filter_enable && (abs_rpm < conf_now->foc_phase_filter_max_erpm))
        {
            ...
            if(!conf_now->foc_phase_filter_disable_fault &&
               (fabsf(v_mag_mod - state_m->v_mag_filter) > (conf_now->l_max_vin * 0.05)))
            {
                mc_interface_set_fault_info("v_mag_mod: %.2f, v_mag_filter: %.2f", 2, v_mag_mod,
                                            state_m->v_mag_filter);
