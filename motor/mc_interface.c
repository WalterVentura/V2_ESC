...
// Private functions
static void update_override_limits(volatile motor_if_state_t* motor,
...
void mc_interface_init(void)
{
    ...
    chThdCreateStatic(sample_send_thread_wa, sizeof(sample_send_thread_wa), NORMALPRIO - 1,
                      sample_send_thread, NULL);
    chThdCreateStatic(fault_stop_thread_wa, sizeof(fault_stop_thread_wa), HIGHPRIO - 3,
                      fault_stop_thread, NULL);
    ...
    switch(motor_now()->m_conf.motor_type)
    {
        ...
        case MOTOR_TYPE_FOC:
            ...
            mcpwm_foc_init((mc_configuration*) &m_motor_1.m_conf,
                           (mc_configuration*) &m_motor_2.m_conf);
#else
            mcpwm_foc_init((mc_configuration*) &m_motor_1.m_conf,
                           (mc_configuration*) &m_motor_1.m_conf);
#endif
            break;
...
const char* mc_interface_fault_to_string(mc_fault_code fault)
{
    switch(fault)
    {
        ...
        case FAULT_CODE_PHASE_FILTER:
            return "FAULT_CODE_PHASE_FILTER";

        case FAULT_CODE_ENCODER_FAULT:
            return "FAULT_CODE_ENCODER_FAULT";

        case FAULT_CODE_LV_OUTPUT_FAULT:
            return "FAULT_CODE_LV_OUTPUT_FAULT";
    }
...
void mc_interface_sample_print_data(debug_sampling_mode mode, uint16_t len, uint8_t decimation,
                                    bool raw, void (* reply_func)(unsigned char* data,
                                                                  unsigned int len))
{
...
{
    const volatile mc_configuration* conf = mc_interface_get_configuration();
    const float tacho_scale = (conf->si_wheel_diameter * M_PI) /
                              (3.0 * conf->si_motor_poles * conf->si_gear_ratio);
...
#ifdef HW_HAS_DUAL_MOTORS
    motor_if_state_t* motor =
        is_second_motor ? (motor_if_state_t*) &m_motor_2 : (motor_if_state_t*) &m_motor_1;
#else
...
// m_phase_samples[m_sample_now] = (uint8_t)(ang / 360.0 * 250.0);
            ...
            {
                m_curr0_samples[m_sample_now] =
                    (int16_t) (mcpwm_detect_currents[mcpwm_get_comm_step() - 1] *
                               (8.0 / FAC_CURRENT));
                m_curr1_samples[m_sample_now] =
                    (int16_t) (mcpwm_detect_currents_diff[mcpwm_get_comm_step() - 1] *
                               (8.0 / FAC_CURRENT));
                                   ...
                                   volatile mc_configuration* conf)
{
...
#ifdef HW_HAS_DUAL_PARALLEL
    UTILS_LP_FAST(motor->m_temp_fet, fmaxf(NTC_TEMP(ADC_IND_TEMP_MOS), NTC_TEMP(
                                               ADC_IND_TEMP_MOS_M2)), 0.1);
#else
    UTILS_LP_FAST(motor->m_temp_fet, NTC_TEMP(
                      is_motor_1 ? ADC_IND_TEMP_MOS : ADC_IND_TEMP_MOS_M2), 0.1);
#endif
    ...
    switch(conf->m_motor_temp_sens_type)
    {
        case TEMP_SENSOR_NTC_10K_25C:
            temp_motor = is_motor_1 ? NTC_TEMP_MOTOR(conf->m_ntc_motor_beta) : NTC_TEMP_MOTOR_2(
                conf->m_ntc_motor_beta);
            break;

        case TEMP_SENSOR_NTC_100K_25C:
            temp_motor =
                is_motor_1 ? NTC100K_TEMP_MOTOR(conf->m_ntc_motor_beta) : NTC100K_TEMP_MOTOR_2(
                    conf->m_ntc_motor_beta);
            break;

        case TEMP_SENSOR_PTC_1K_100C:
            temp_motor =
                is_motor_1 ? PTC_TEMP_MOTOR(1000.0, conf->m_ptc_motor_coeff,
                                            100) : PTC_TEMP_MOTOR_2(1000.0,
                                                                    conf->m_ptc_motor_coeff,
                                                                    100);
            break;
        ...
        case TEMP_SENSOR_KTY83_122:
        {
            ...
            float res =
                NTC_RES_MOTOR(ADC_Value[is_motor_1 ? ADC_IND_TEMP_MOTOR : ADC_IND_TEMP_MOTOR_2]);
            float pow2 = res * res;
        ...
        case TEMP_SENSOR_NTCX:
            temp_motor = is_motor_1 ? NTCX_TEMP_MOTOR(conf->m_ntcx_ptcx_res, conf->m_ntc_motor_beta,
                                                      conf->m_ntcx_ptcx_temp_base) :
                         NTCX_TEMP_MOTOR_2(conf->m_ntcx_ptcx_res, conf->m_ntc_motor_beta,
                                           conf->m_ntcx_ptcx_temp_base);
            break;

        case TEMP_SENSOR_PTCX:
            temp_motor = is_motor_1 ? PTC_TEMP_MOTOR(conf->m_ntcx_ptcx_res, conf->m_ptc_motor_coeff,
                                                     conf->m_ntcx_ptcx_temp_base) :
                         PTC_TEMP_MOTOR_2(conf->m_ntcx_ptcx_res, conf->m_ptc_motor_coeff,
                                          conf->m_ntcx_ptcx_temp_base);
            break;
    ...
    if(UTILS_IS_NAN(temp_motor) || UTILS_IS_INF(temp_motor) || (temp_motor > 600.0) ||
       (temp_motor < -200.0))
    {
        temp_motor = -100.0;
    }
    ...
    else
    {
        ...
        maxc =
            utils_map(motor->m_temp_fet, conf->l_temp_fet_start, conf->l_temp_fet_end, maxc, 0.0);
    ...
    else
    {
        ...
        maxc = utils_map(motor->m_temp_motor, conf->l_temp_motor_start, conf->l_temp_motor_end,
                         maxc, 0.0);
    ...
    const float temp_fet_accel_start = utils_map(conf->l_temp_accel_dec, 0.0, 1.0,
                                                 conf->l_temp_fet_start, 25.0);
    const float temp_fet_accel_end = utils_map(conf->l_temp_accel_dec, 0.0, 1.0,
                                               conf->l_temp_fet_end, 25.0);
    const float temp_motor_accel_start = utils_map(conf->l_temp_accel_dec, 0.0, 1.0,
                                                   conf->l_temp_motor_start, 25.0);
    const float temp_motor_accel_end = utils_map(conf->l_temp_accel_dec, 0.0, 1.0,
                                                 conf->l_temp_motor_end, 25.0);
...
static void run_timer_tasks(volatile motor_if_state_t* motor)
{
    ...
    UTILS_LP_FAST(motor->m_input_voltage_filtered_slower, motor->m_input_voltage_filtered,
                  voltage_fc);
    ...
    switch(motor->m_conf.m_out_aux_mode)
    {
        ...
        case OUT_AUX_MODE_MOSFET_50:
            ...
            if(mc_interface_temp_fet_filtered() > 50.0)
            {
                AUX_ON();
            }
            else
        ...
        case OUT_AUX_MODE_MOSFET_70:
            ...
            if(mc_interface_temp_fet_filtered() > 70.0)
            {
                AUX_ON();
            }
            else
        ...
        case OUT_AUX_MODE_MOTOR_MOSFET_50:
            ...
            if((mc_interface_temp_motor_filtered() > 50.0) ||
               (mc_interface_temp_fet_filtered() > 50.0))
            {
                AUX_ON();
            }
            else
        ...
        case OUT_AUX_MODE_MOTOR_MOSFET_70:
            ...
            if((mc_interface_temp_motor_filtered() > 70.0) ||
               (mc_interface_temp_fet_filtered() > 70.0))
            {
                AUX_ON();
            }
            else
...
static THD_FUNCTION(sample_send_thread, arg)
{
    ...
    for(;;)
    {
        ...
        for(int i = 0; i < len; i++)
        {
            ...
            {
                buffer_append_float32_auto(buffer,
                                           (float) m_curr0_samples[ind_samp] / (8.0 / FAC_CURRENT),
                                           &index);
                buffer_append_float32_auto(buffer,
                                           (float) m_curr1_samples[ind_samp] / (8.0 / FAC_CURRENT),
                                           &index);
                buffer_append_float32_auto(buffer,
                                           (float) m_curr2_samples[ind_samp] / (8.0 / FAC_CURRENT),
                                           &index);
                buffer_append_float32_auto(buffer,
                                           ((float) m_ph1_samples[ind_samp] / 4096.0 * V_REG) *
                                           ((VIN_R1 + VIN_R2) / VIN_R2) * ADC_VOLTS_PH_FACTOR,
                                           &index);
                buffer_append_float32_auto(buffer,
                                           ((float) m_ph2_samples[ind_samp] / 4096.0 * V_REG) *
                                           ((VIN_R1 + VIN_R2) / VIN_R2) * ADC_VOLTS_PH_FACTOR,
                                           &index);
                buffer_append_float32_auto(buffer,
                                           ((float) m_ph3_samples[ind_samp] / 4096.0 * V_REG) *
                                           ((VIN_R1 + VIN_R2) / VIN_R2) * ADC_VOLTS_PH_FACTOR,
                                           &index);
                buffer_append_float32_auto(buffer,
                                           ((float) m_vzero_samples[ind_samp] / 4096.0 * V_REG) *
                                           ((VIN_R1 + VIN_R2) / VIN_R2) * ADC_VOLTS_INPUT_FACTOR,
                                           &index);
                buffer_append_float32_auto(buffer,
                                           (float) m_curr_fir_samples[ind_samp] / (8.0 / FAC_CURRENT),
                                           &index);
            }
...
static THD_FUNCTION(fault_stop_thread, arg)
{
    ...
    for(;;)
    {
        ...
        volatile motor_if_state_t* motor =
            fault_data_copy.is_second_motor ? &m_motor_2 : &m_motor_1;
#else
