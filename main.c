...
int main(void)
{
    ...
    chThdCreateStatic(periodic_thread_wa, sizeof(periodic_thread_wa), NORMALPRIO, periodic_thread,
                      NULL);
    ...
    for(;;)
    {
        ...
        switch(display_mode)
        {
            ...
            case DISP_POS_MODE_PID_POS_ERROR:
                commands_send_rotor_pos(utils_angle_difference(mc_interface_get_pid_pos_set(),
        ...
        if(mc_interface_get_configuration()->motor_type == MOTOR_TYPE_FOC)
        {
            switch(display_mode)
            {
                ...
                case DISP_POS_MODE_ENCODER_OBSERVER_ERROR:
                    commands_send_rotor_pos(utils_angle_difference(mcpwm_foc_get_phase_observer(),
                                                                   mcpwm_foc_get_phase_encoder()));
                    break;

                case DISP_POS_MODE_HALL_OBSERVER_ERROR:
                    commands_send_rotor_pos(utils_angle_difference(mcpwm_foc_get_phase_observer(),
                                                                   mcpwm_foc_get_phase_hall()));
                    break;
