...
#define APP_SIZE                            ((uint32_t) (APP_MAX_SIZE - VECTOR_TABLE_SIZE - \
...
#define ERASE_VOLTAGE_RANGE                 (uint8_t) ((PWR->CSR &                       \
                                                        PWR_CSR_PVDO) ? VoltageRange_2 : \
                                                       VoltageRange_3)
...
uint32_t flash_helper_verify_flash_memory(void)
{
    ...
    else
    {
        ...
        uint16_t res = FLASH_ProgramWord((uint32_t) APP_CRC_WAS_CALCULATED_FLAG_ADDRESS,
                                         APP_CRC_WAS_CALCULATED_FLAG);
